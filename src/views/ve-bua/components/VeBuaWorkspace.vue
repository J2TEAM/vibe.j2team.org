...
const STARTER_BUA_NAME_SUFFIXES = ['pass test', 'giải bug', 'fix nóng', 'merge êm', 'chống conflict', 'đỡ toang', 'deploy mượt', 'chịu tải']
const CHANT_SAMPLES = [
...
const CHANT_PLACEHOLDER = 'Ví dụ: Xin cho mọi thứ thuận lợi, không bug production, công việc suôn sẻ.'
...
function isValidLegacyPayload(payload: BuaExportPayload): boolean {
  ...
  if (payload.settings.brushOpacity !== undefined && !Number.isFinite(payload.settings.brushOpacity)) return false
  if (!Number.isFinite(payload.settings.axisCount)) return false
  if (payload.settings.brushSizeRandomness !== undefined && !Number.isFinite(payload.settings.brushSizeRandomness)) return false
  if (payload.settings.brushOpacityRandomness !== undefined && !Number.isFinite(payload.settings.brushOpacityRandomness)) return false
  ...
  if (payload.settings.paperTransform) {
    ...
    if (!Number.isFinite(transform.offsetX) || !Number.isFinite(transform.offsetY) || !Number.isFinite(transform.scale)) return false
  }
...
function isValidStylePayload(payload: BuaStylePayload): boolean {
  ...
  if (payload.style.brushOpacity !== undefined && !Number.isFinite(payload.style.brushOpacity)) return false
  if (!Number.isFinite(payload.style.axisCount)) return false
  if (payload.style.brushSizeRandomness !== undefined && !Number.isFinite(payload.style.brushSizeRandomness)) return false
  if (payload.style.brushOpacityRandomness !== undefined && !Number.isFinite(payload.style.brushOpacityRandomness)) return false
  ...
  return isValidSnapshot({ drawingDataUrl: '', center: payload.axis.center, axisAngles: payload.axis.axisAngles })
}

function isValidDrawingPayload(payload: BuaDrawingPayload): boolean {
  return payload.version === 1 && payload.name.length > 0 && typeof payload.drawingDataUrl === 'string'
}
...
function loadCollection(): CollectionItem[] {
  try {
    ...
    if (v2Raw) {
      const v2Items = JSON.parse(v2Raw) as Array<{ id: string; name: string; code: string; createdAt: string }>
      const migratedV2: CollectionItem[] = v2Items
        .map((item) => {
          ...
          const stylePayload: BuaStylePayload = {
            ...
            axis: { center: { ...legacy.snapshot.center }, axisAngles: legacy.snapshot.axisAngles.slice() },
          }
...
function randomStarterName(): string {
  const prefix = STARTER_BUA_NAME_PREFIXES[Math.floor(Math.random() * STARTER_BUA_NAME_PREFIXES.length)] ?? 'Bùa'
  const suffix = STARTER_BUA_NAME_SUFFIXES[Math.floor(Math.random() * STARTER_BUA_NAME_SUFFIXES.length)] ?? 'pass test'
  ...
  return Array.from(names).map((name, index) => {
    ...
    return {
      ...
      styleCode: encodeStyleCode({
        ...
        axis: { center: { ...payload.snapshot.center }, axisAngles: payload.snapshot.axisAngles.slice() },
      }),
...
async function applyStylePayload(payload: BuaStylePayload) {
  ...
  brushOpacityRandomness.value = Math.min(100, Math.max(0, payload.style.brushOpacityRandomness ?? 50))
  ...
  paperTransform.value = sanitizePaperTransform(payload.style.paperTransform ?? { offsetX: 0, offsetY: 0, scale: 1 })
  savePaperTransform()
  await nextTick()
...
async function importFromCodeText(rawCode: string) {
  ...
  await applyStylePayload({
    ...
    axis: { center: { ...legacyPayload.snapshot.center }, axisAngles: legacyPayload.snapshot.axisAngles.slice() },
  })
...
async function loadFromCollection(item: CollectionItem, mode: 'style' | 'drawing' | 'both') {
  ...
  showCollection.value = false
  infoText.value = mode === 'both' ? `Đã nạp đầy đủ: ${item.name}` : mode === 'style' ? `Đã nạp style: ${item.name}` : `Đã nạp nét vẽ: ${item.name}`
}
...
<template>
  <div class="relative h-screen w-full overflow-hidden bg-bg-deep text-text-primary">
    ...
    <div v-if="!isBurning && !isPostBurnHolding" class="absolute left-3 top-3 z-40 flex flex-col items-start gap-2">
      <div class="flex items-center gap-2">
        ...
        <button
          ...
          :class="showBrushPanel ? 'border-white bg-white text-black' : 'border-border-default bg-bg-surface'"
          title="Bút"
        ...
        <button
          ...
          :class="showMokPanel ? 'border-white bg-white text-black' : 'border-border-default bg-bg-surface text-text-secondary'"
          title="Gõ mõ"
        ...
        <button
          ...
          :class="showAxisPanel ? 'border-white bg-white text-black' : 'border-border-default bg-bg-surface'"
          title="Trục"
        ...
        <button
          ...
          :class="showDrawablePanel ? 'border-white bg-white text-black' : 'border-border-default bg-bg-surface'"
          title="Vùng vẽ"
    ...
    <div v-if="!isBurning && !isPostBurnHolding" class="absolute right-3 top-3 z-40 flex flex-col items-end gap-2">
      <div class="pointer-events-auto flex flex-wrap items-center justify-end gap-2">
        <button
          ...
          :class="showMenu ? 'border-white bg-white text-black' : 'border-border-default bg-bg-surface'"
        ...
        <button
          class="h-10 border px-3 text-xs font-display tracking-wide text-text-secondary transition hover:border-accent-amber hover:text-text-primary"
          :class="showChantPanel ? 'border-white bg-white text-black' : 'border-border-default bg-bg-surface'"
          title="Khấn"
      ...
      <div v-if="showTransferActions" class="pointer-events-auto w-[220px] border border-border-default bg-bg-surface p-2">
        <button
    ...
    <main class="relative z-20 h-full w-full px-3 pb-3 pt-20 sm:px-4 sm:pb-4 sm:pt-24">
      <div
        ...
        style="background: radial-gradient(circle at center, rgb(15 25 35 / 0.15) 0%, rgb(15 25 35 / 0.68) 58%, rgb(15 25 35 / 0.85) 100%)"
      />
      <div
    ...
    <BuaExportModal :open="showExportModal" :mode="exportMode" :code="exportCodeText" @close="showExportModal = false" @copy="copyExportCode" />
