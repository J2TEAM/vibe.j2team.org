  ...
  <section v-if="props.open" class="absolute inset-0 z-50 bg-bg-deep p-4 sm:p-6">
    <div class="mx-auto flex h-full w-full max-w-6xl flex-col gap-4">
      <div class="flex items-center justify-between border border-border-default bg-bg-surface px-4 py-3">
        <h2 class="font-display text-sm uppercase tracking-wider text-text-primary">Bộ sưu tập bùa</h2>
        <button
      ...
      <div v-if="props.items.length === 0" class="border border-border-default bg-bg-surface p-6 text-sm text-text-secondary">
        Bộ sưu tập trống.
      </div>

      <div v-else class="grid flex-1 grid-cols-2 gap-3 overflow-auto pr-1 sm:grid-cols-3 lg:grid-cols-4">
        <article
          v-for="item in props.items"
          :key="item.id"
          class="border border-border-default bg-bg-surface p-2"
        >
          <button class="block w-full text-left" @click="emit('load', { id: item.id, name: item.name, styleCode: item.styleCode, drawingCode: item.drawingCode, createdAt: item.createdAt }, 'both')">
            <div
            ...
            <p class="text-[11px] text-text-dim">{{ new Date(item.createdAt).toLocaleString('vi-VN') }}</p>
          </button>
          <div class="mt-2 grid grid-cols-3 gap-2">
            <button
              ...
              @click="emit('load', { id: item.id, name: item.name, styleCode: item.styleCode, drawingCode: item.drawingCode, createdAt: item.createdAt }, 'style')"
            >
              Nạp style
            </button>
            <button
              ...
              @click="emit('load', { id: item.id, name: item.name, styleCode: item.styleCode, drawingCode: item.drawingCode, createdAt: item.createdAt }, 'drawing')"
            >
              Nạp nét
            </button>
            <button
              ...
              @click="emit('load', { id: item.id, name: item.name, styleCode: item.styleCode, drawingCode: item.drawingCode, createdAt: item.createdAt }, 'both')"
            >
              Nạp cả 2
            </button>
          </div>
