...
<template>
  <div v-if="props.open" class="absolute inset-y-0 left-0 z-40 w-[min(300px,85vw)] overflow-auto pr-2">
    <BuaControlPanel side="left" title="Bút và nền">
      ...
      <label class="block">
        <span class="mb-1 block text-xs uppercase tracking-wider text-text-dim">Độ dày nét: {{ props.brushSize }}</span>
        <input
      ...
      <label class="block">
        <span class="mb-1 block text-xs uppercase tracking-wider text-text-dim">Độ đậm bút: {{ localBrushOpacity }}%</span>
        <input
      ...
      <label class="block">
        <span class="mb-1 block text-xs uppercase tracking-wider text-text-dim">Ngẫu nhiên nét to/nhỏ: {{ props.brushSizeRandomness }}%</span>
        <input
          ...
          step="1"
          class="w-full accent-[#FF6B4A]"
          @input="emit('update:brushSizeRandomness', toNumber(($event.target as HTMLInputElement).value))"
        />
      </label>

      <label class="block">
        <span class="mb-1 block text-xs uppercase tracking-wider text-text-dim">Ngẫu nhiên nét đậm/nhạt: {{ props.brushOpacityRandomness }}%</span>
        <input
          ...
          step="1"
          class="w-full accent-[#FFB830]"
          @input="emit('update:brushOpacityRandomness', toNumber(($event.target as HTMLInputElement).value))"
        />
      </label>

      <label class="flex items-center justify-between gap-2 border border-border-default px-3 py-2 text-sm">
