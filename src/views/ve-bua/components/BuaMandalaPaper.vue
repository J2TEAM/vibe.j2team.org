...
function drawSegment(ctx: CanvasRenderingContext2D, from: Point, to: Point, color: string) {
  ...
  for (let i = 0; i < layers; i += 1) {
    ...
    const opacity = Math.min(0.98, (minOpacity + Math.random() * (maxOpacity - minOpacity)) * baseOpacity)
...
function beginDrawing(event: PointerEvent) {
  if (isBurning.value || props.showPaperTransformEditor || activeDrag.value !== 'none' || drawableDragMode.value !== 'none') return
...
function continueDrawing(event: PointerEvent) {
  if (!isDrawing.value || isBurning.value || props.showPaperTransformEditor || activeDrag.value !== 'none' || drawableDragMode.value !== 'none') return
  const localPoint = getLocalPoint(event)
  if (!localPoint) return
  const canvasPoint = toCanvasPoint(localPoint)
  if (!canvasPoint) return
...
function startPaperScale(event: PointerEvent) {
  ...
  paperTransformStartDistance.value = Math.max(24, Math.hypot(event.clientX - centerClient.x, event.clientY - centerClient.y))
}
...
function movePaperTransform(event: PointerEvent) {
  ...
  const nextDistance = Math.max(24, Math.hypot(event.clientX - centerClient.x, event.clientY - centerClient.y))
...
function buildHueFilter(targetHex: string, baseHue: number, satFactor: number, lightFactor: number): Record<string, string> {
...
watch(
  () => [props.drawableBox.x, props.drawableBox.y, props.drawableBox.width, props.drawableBox.height],
  () => {
    resizeCanvas()
  },
)
    ...
    >
      <div class="absolute inset-0 overflow-hidden" :style="artifactLayerStyle">
        <div class="absolute inset-0 overflow-hidden">
          <img class="h-full w-full object-cover" :src="nenGiayImage" :style="paperImageStyle" alt="Nền giấy bùa" draggable="false" />
        </div>
        ...
        <div class="pointer-events-none absolute inset-0 overflow-hidden">
          <img class="h-full w-full object-cover" :src="vienBuaImage" :style="frameImageStyle" alt="Viền bùa" draggable="false" />
        </div>
      </div>
      ...
      <template v-if="props.showDrawableEditor">
        ...
        <button
          data-ui-handle="1"
          class="absolute h-4 w-4 -translate-x-1/2 -translate-y-1/2 border border-accent-sky bg-bg-surface"
          :style="{ left: `${(props.drawableBox.x + props.drawableBox.width) * 100}%`, top: `${props.drawableBox.y * 100}%` }"
          @pointerdown="startDrawableDrag('ne', $event)"
        />
        <button
          data-ui-handle="1"
          class="absolute h-4 w-4 -translate-x-1/2 -translate-y-1/2 border border-accent-sky bg-bg-surface"
          :style="{ left: `${props.drawableBox.x * 100}%`, top: `${(props.drawableBox.y + props.drawableBox.height) * 100}%` }"
          @pointerdown="startDrawableDrag('sw', $event)"
        />
        <button
          data-ui-handle="1"
          class="absolute h-4 w-4 -translate-x-1/2 -translate-y-1/2 border border-accent-sky bg-bg-surface"
          :style="{ left: `${(props.drawableBox.x + props.drawableBox.width) * 100}%`, top: `${(props.drawableBox.y + props.drawableBox.height) * 100}%` }"
...
.burn-trail {
  ...
  background: linear-gradient(to top, rgb(255 107 74 / 0.9), rgb(255 184 48 / 0.8), rgb(255 232 171 / 0));
  filter: blur(0.25px);
