...
<template>
  <div v-if="props.open" class="absolute inset-y-0 right-0 z-40 w-[min(320px,85vw)] overflow-auto pl-2">
    <BuaControlPanel side="right" title="Mã bùa và thao tác">
      <label class="block">
        <span class="mb-1 block text-xs uppercase tracking-wider text-text-dim">Tên thiết kế để lưu</span>
        <input
      ...
      <p v-if="props.infoText" class="border border-border-default px-3 py-2 text-xs text-text-secondary">
        {{ props.infoText }}
      </p>
    </BuaControlPanel>
  </div>
