...
<template>
  <div
    class="grid grid-cols-3 sm:flex sm:flex-col gap-3 w-full sm:w-32 shrink-0 p-2 border border-border-default bg-bg-surface overflow-y-auto scrollbar-thin scrollbar-thumb-border-default"
  >
    <div
    ...
    >
      ...
      <span
        class="absolute -bottom-2 -right-1 font-pixel text-4xl font-bold text-white/5 select-none pointer-events-none group-hover:text-accent-coral/10 transition-colors"
      >
        0{{ agents.indexOf(agent) + 1 }}
      </span>

      <div
        class="font-pixel text-base sm:text-lg text-text-primary font-bold uppercase tracking-widest mb-2 group-hover:text-accent-coral transition-colors"
      >
        {{ agent.name }}
      </div>
      ...
      <div class="h-10 flex items-center justify-center">
        ...
        <div
          v-else
          class="w-1.5 h-1.5 bg-accent-amber animate-pulse shadow-[0_0_8px_rgba(255,184,48,0.5)]"
        ></div>
      </div>
    </div>
  </div>
