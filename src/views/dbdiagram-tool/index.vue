...
const activeDialect = ref<'mysql' | 'postgresql' | 'sqlite'>('mysql')
const sqlCopied = ref(false)

// AI Store
const aiStore = useAiSettingsStore()
const aiPanelOpen = ref(false)
const aiProvider = ref<'gemini' | 'openai' | 'claude' | 'openrouter-gemini'>('openrouter-gemini')
...
const aiProviders = [
  ...
  { id: 'openrouter-gemini' as const, label: 'OpenRouter' },
]
...
async function generateWithAI() {
  ...
  try {
    ...
    } else if (aiProvider.value === 'openrouter-gemini') {
      const { OpenRouter } = await import('@openrouter/sdk')
      const client = new OpenRouter({ apiKey: aiApiKey.value })

      const resultModel = client.callModel({
        model: aiStore.DEFAULT_MODEL,
        instructions: AI_SYSTEM,
        input: [{ role: 'user', content: msg }],
      })

      dbmlInput.value = '' // Clear and start streaming
      for await (const delta of resultModel.getTextStream()) {
        dbmlInput.value += delta
      }
      result = dbmlInput.value
...
function downloadSql() {
  ...
  a.href = url
  a.download = `schema_${activeDialect.value}_${Date.now()}.sql`
