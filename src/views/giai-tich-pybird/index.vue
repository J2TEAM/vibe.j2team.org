  ...
  <div class="game-container">
    ...
    <div class="game-wrapper" @contextmenu.prevent>
      ...
      <div v-if="gameState === 'START'" class="overlay">
        ...
        <p>
          Thí sinh: <b>{{ playerName }}</b>
        </p>
        <p>
          Kỷ lục cá nhân: <b style="color: #2ecc71; font-size: 18px">{{ highScore }}</b> câu
        </p>
        <p>
          Hệ thống: <b>Adaptive Random</b> | Phao: <b>{{ cheats }}</b>
        </p>
      ...
      <div v-if="gameState === 'GAMEOVER'" class="overlay gameover-screen">
        ...
        <p>
          Số câu sống sót: <b>{{ score }}</b>
        </p>
        <p
          v-if="score >= highScore && score > 0"
          style="color: #f1c40f; font-weight: bold; animation: popIn 0.5s"
        >
          🎉 KỶ LỤC MỚI! 🎉
        </p>
      ...
      <div v-if="gameState === 'QUESTION' && currentQuestion" class="question-modal">
        <div class="question-box">
          <div class="question-header">
            ...
            <button v-if="cheats > 0" @click="useCheat" class="btn-cheat">
              Dùng Phao ({{ cheats }})
            </button>
          </div>
...
const generateQuestion = (currentScore: number): Question => {
  ...
  const easyTemplates = [
    (): Question => {
      ...
      return {
        type: 'mcq',
        q: `Tính giới hạn: lim(x->0) [sin(${a}x) / ${b}x]`,
        options,
        ans: options.indexOf(ansStr),
      }
    },
    (): Question => {
      const a = randInt(2, 6)
      ...
      return {
        type: 'short',
        q: `Cho hàm số f(x) = ${a}x^${b} + x. Đạo hàm f'(1) bằng?`,
        ans: (ans + 1).toString(),
      }
    },
    (): Question => {
  ...
  const mediumTemplates = [
    (): Question => {
      ...
      return {
        type: 'mcq',
        q: `Tính tích phân I = ∫(0 đến 2) ${a}x dx`,
        options,
    ...
    (): Question => {
      ...
      return {
        type: 'short',
        q: `Cho f(x,y) = ${a}x^2 + ${b}y^2. Tính đạo hàm riêng f'_x tại điểm (1, 1).`,
        ans: ans.toString(),
      }
    },
    (): Question => {
      ...
      return {
        type: 'tf',
        q: `Bán kính hội tụ R của chuỗi lũy thừa ∑ (x^n) / ${a}^n là R = ${a}.`,
        ans: true,
      }
    },
  ]
  ...
  const hardTemplates = [
    (): Question => {
      ...
      const options = shuffleArray([
        ...
        `y = C1*cos(${r1}x) + C2*sin(${r2}x)`,
        'y = C1*x + C2',
      ])
      return {
        type: 'mcq',
        q: `Nghiệm tổng quát của PT vi phân: y'' - ${S}y' + ${P}y = 0 là?`,
        options,
        ans: options.indexOf(ansStr),
      }
    },
    (): Question => {
      ...
      return {
        type: 'tf',
        q: `Diện tích hình tròn x^2 + y^2 <= ${a}^2 tính bằng tích phân kép là ${ans}π.`,
        ans: false,
      }
    },
    (): Question => {
      ...
      return {
        type: 'short',
        q: `Hệ số a_0 trong chuỗi Fourier của hàm lẻ f(x) = ${a}x trên (-π, π) bằng?`,
        ans: '0',
      }
    },
  ]
