    ...
    zones: [
      {
        ...
        xStart: 200,
        xEnd: 1000,
        monsterType: 'frost_spirit',
        maxMonsters: 3,
        spawnRate: 150,
        label: '❄️ Hồ Đóng Băng',
        color: '#bfdbfe',
      },
      {
        name: 'Hang Tuyết',
...
export function drawMapSelection(
  ...
  allMaps.forEach((mapId, i) => {
    ...
    ctx.strokeStyle = isSelected ? '#818cf8' : '#2a2a3e'
    ctx.lineWidth = isSelected ? 2 : 1
    ctx.strokeRect(px + 12, iy, panelW - 24, 52)

    if (isSelected) {
      ctx.fillStyle = '#818cf8'
      ctx.fillRect(px + 12, iy, 3, 52)
    }
...
export function drawReturnHint(
  ctx: CanvasRenderingContext2D,
  x: number,
  y: number,
  cameraX: number,
