...
export function drawUpgradePanel(
  ...
  if (!selectedEquip) {
    ...
    if (equippedWing) {
      ...
      ctx.fillText(
        `${equippedWing.icon} ${equippedWing.name} (Cánh)`,
        listX + 4,
        startY + row * (itemH + 4) + 14,
      )
      row++
    }
    ...
    if (equipInventory.length > 0) {
      ...
      if (equipInventory.length > 6) {
        ...
        ctx.font = '9px monospace'
        ctx.fillText(
          `... và ${equipInventory.length - 6} vật phẩm nữa`,
          listX + 4,
          startY + row * (itemH + 4) + 14,
        )
      }
  ...
  if (eq.level < maxLevel) {
    ...
    let nextStr = '→ '
    if (eq.stats.hp)
      nextStr += `HP+${Math.round((eq.stats.hp * boost) / (1 + (eq.level - 1) * 0.15))} `
    if (eq.stats.atk)
      nextStr += `ATK+${Math.round((eq.stats.atk * boost) / (1 + (eq.level - 1) * 0.15))} `
    if (eq.stats.def)
      nextStr += `DEF+${Math.round((eq.stats.def * boost) / (1 + (eq.level - 1) * 0.15))} `
  ...
  ctx.font = '10px monospace'
  ctx.fillText(
    `${useRefineDust ? '☑' : '☐'} ✨ Bụi Tinh Luyện (+15%) ×${materials.refine_dust}`,
    itemX + 6,
    optY + 18,
  )
  ...
  ctx.fillText(
    `${useProtection ? '☑' : '☐'} 📜 Cuộn Bảo Vệ ×${materials.protection_scroll}`,
    itemX + 216,
    optY + 18,
  )
  ...
  ctx.textAlign = 'center'
  ctx.fillText(
    canUpgrade ? '⬆ NÂNG CẤP' : eq.level >= maxLevel ? 'MAX LEVEL' : 'THIẾU NGL',
    px + panelW / 2,
    btnY + 24,
  )
  ctx.textAlign = 'left'
  ...
  ctx.textAlign = 'center'
  ctx.fillText(
    'Enter: Nâng cấp | D/F: Bật/tắt Bụi/Cuộn | Esc: Quay lại',
    px + panelW / 2,
    py + panelH - 12,
  )
  ctx.textAlign = 'left'
...
export function drawWingHUD(
  ctx: CanvasRenderingContext2D,
  wing: WingConfig | null,
  x: number,
  y: number,
) {
  if (!wing) return
