...
export function drawNPC(ctx: CanvasRenderingContext2D, npc: NPC, cameraX: number, playerX: number) {
  ...
  if (npc.type === 'merchant') {
    ...
    ctx.beginPath()
    ctx.arc(npc.w / 2, npc.h / 2, 10, 0, Math.PI * 2)
    ctx.fill()
    // Sparkles
    for (let i = 0; i < 4; i++) {
...
export function drawShopUI(
  ...
  items.forEach((item, i) => {
    ...
    ctx.fillStyle = canAfford ? '#e2e8f0' : '#6b7280'
    ctx.font = '8px monospace'
...
export function drawUpgradeUI(
  ...
  if (!selectedEquip) {
    ...
    for (let i = startI; i < endI; i++) {
      ...
      ctx.strokeStyle = isSelected ? color : '#2a2a3e'
      ...
      ctx.font = '8px monospace'
      ctx.fillText(
        `${RARITY_NAMES[eq.rarity]} | Lv.${eq.level}/10 | ${SLOT_ICONS[eq.slot]}`,
        px + 44,
        iy + 26,
      )
      ...
      if (eq.level >= 10) {
        ...
        ctx.textAlign = 'right'
        ctx.fillText('MAX', px + panelW - 20, iy + 20)
        ctx.textAlign = 'left'
      }
    }
  ...
  if (upgradeAnim > 0) {
    ...
    ctx.fillStyle =
      upgradeResult === 'success' ? `rgba(74,222,128,${a * 0.3})` : `rgba(239,68,68,${a * 0.3})`
    ctx.fillRect(px + 12, py + 38, panelW - 24, 60)
  }
  ...
  ctx.fillRect(barX, barY2, barW * (rate / 100), barH)
  ctx.strokeStyle = '#374151'
  ctx.lineWidth = 1
  ...
  ctx.fillStyle = hasDust ? '#a78bfa' : '#4b5563'
  ctx.font = '9px monospace'
  ctx.fillText(
    `${useRefineDust ? '☑' : '☐'} ✨ +15% (×${materials.refine_dust})`,
    px + 22,
    optY + 17,
  )
  ...
  ctx.fillStyle = hasScroll ? '#fbbf24' : '#4b5563'
  ctx.font = '9px monospace'
  ctx.fillText(
    `${useProtection ? '☑' : '☐'} 📜 Bảo vệ (×${materials.protection_scroll})`,
    optX2 + 6,
    optY + 17,
  )
  ...
  ctx.strokeStyle = canDo ? '#fbbf24' : '#4b5563'
  ctx.lineWidth = 2
  ctx.strokeRect(px + panelW / 2 - 90, btnY, 180, 34)
  ctx.fillStyle = canDo ? '#fbbf24' : '#4b5563'
  ...
  ctx.textAlign = 'center'
  ctx.fillText(
    'Click nâng cấp | Click ✨/📜 bật/tắt | Esc quay lại',
    px + panelW / 2,
    py + panelH - 8,
  )
  ctx.textAlign = 'left'
