...
export function getRandomWingDrop(monsterType: string): WingType | null {
  ...
  if (monsterType === 'boss') {
    weights.dragon_wings += 15
    weights.celestial_wings += 10
  }
...
export function drawWings(
  ...
  const t = Date.now()
  const flap = isFlying ? Math.sin(t * 0.015) * 0.5 : Math.sin(t * 0.003 + frame * 0.1) * 0.15
  const yOff = h * 0.3
  ...
  if (wing.id === 'wood_wings') {
    ...
    ctx.beginPath()
    ctx.ellipse(w / 2, yOff, span + 4, 12, 0, 0, Math.PI * 2)
    ctx.fill()
    ...
    const drawShadowWing = (dir: number) => {
      ...
      for (let layer = 3; layer >= 0; layer--) {
        ...
        ctx.quadraticCurveTo(
          dir * layerSpan * 0.5,
          -layerSpan * 0.6 + waveOff - flap * 10,
          dir * layerSpan,
          -layerSpan * 0.1 + waveOff,
        )
        ctx.quadraticCurveTo(dir * layerSpan * 0.6, layerSpan * 0.2 + waveOff, 0, 6)
        ctx.fill()
      }
      ctx.restore()
    }
    ...
    ctx.beginPath()
    ctx.ellipse(w / 2, yOff, span + 8, 18, 0, 0, Math.PI * 2)
    ctx.fill()
  }
}
...
export function drawWingDrop(
  ...
  ctx.fillStyle = `${color}${Math.floor(alpha * 255)
    ...
    .padStart(2, '0')}`
  ctx.beginPath()
  ctx.arc(dx + 10, y + 10 + bob, 16, 0, Math.PI * 2)
  ctx.fill()

  ctx.fillStyle = 'rgba(15,15,35,0.9)'
  ctx.beginPath()
  ctx.arc(dx + 10, y + 10 + bob, 11, 0, Math.PI * 2)
  ctx.fill()
  ...
  if (life < 120 && Math.floor(Date.now() / 200) % 2 === 0) {
    ctx.textAlign = 'left'
    return
  }
