...
export const MATERIALS: Record<MaterialType, MaterialConfig> = {
  upgrade_stone: {
    ...
    icon: '💎',
    color: '#60a5fa',
    description: 'Nguyên liệu cơ bản để nâng cấp trang bị',
    shopPrice: 100,
  },
  refine_dust: {
    id: 'refine_dust',
    ...
    icon: '✨',
    color: '#a78bfa',
    description: 'Tăng 15% tỷ lệ thành công nâng cấp',
    shopPrice: 250,
  },
  protection_scroll: {
    ...
    icon: '📜',
    color: '#fbbf24',
    description: 'Bảo vệ trang bị không giảm cấp khi thất bại',
    shopPrice: 500,
  },
}
...
export function rollMaterialDrop(monsterType: string): MaterialType | null {
  const chance = monsterType === 'boss' ? 0.6 : monsterType === 'demon' ? 0.15 : 0.06
  if (Math.random() > chance) return null
  const roll = Math.random()
  if (roll < 0.6) return 'upgrade_stone'
  if (roll < 0.9) return 'refine_dust'
