...
import {
  ...
  PLAYER_SPAWN_INVINCIBLE,
  FRICTION,
  MAX_JUMPS,
  INVINCIBLE_FRAMES,
  DASH_INVINCIBLE_FRAMES,
  HURT_TIMER_FRAMES,
  MINOR_HURT_TIMER,
  CRIT_MULT_DEFAULT,
  CRIT_MULT_BURST,
  ...
  LIGHTNING_CHANCE,
  LIGHTNING_MULT,
  MANA_SHIELD_REDUCTION,
  MANA_SHIELD_COST_MULT,
  SHIELD_DAMAGE_MULT,
  KNOCKBACK_VX,
  KNOCKBACK_VY,
  MONSTER_ATTACK_TIMER,
  MONSTER_ATTACK_COOLDOWN,
  MONSTER_HIT_KNOCKBACK,
  SCORE_COMBO_SCALE,
  ATTACK_TIMER,
  ...
  LEVEL_SPEED_BONUS,
  SHAKE_INTENSITY_HIT,
  SHAKE_INTENSITY_CRIT,
  SHAKE_INTENSITY_BOSS,
  SHAKE_DECAY,
  COMBO_TIMEOUT,
  COMBO_DMG_SCALE,
  COMBO_MAX_MULT,
  ...
  BOSS_PHASE2_SPEED,
  BOSS_PHASE3_SPEED,
  BOSS_PHASE1_COOLDOWN,
  BOSS_PHASE2_COOLDOWN,
  BOSS_PHASE3_COOLDOWN,
  AUTOSAVE_KEY,
  AUTOSAVE_INTERVAL,
...
import {
  ...
  spawnFloatingText as _spawnFloatingText,
  updateParticles as _updateParticles,
  updateFloatingTexts as _updateFloatingTexts,
  drawParticles as _drawParticles,
  drawFloatingTexts as _drawFloatingTexts,
} from './rendering/particles'
// World
...
function onKeyDown(e: KeyboardEvent) {
  ...
  if (e.code === 'Tab' && showInventory.value) {
    e.preventDefault()
    invTab = (invTab + 1) % 4
  }
  // Quick equip 1-6
    ...
    for (let i = 0; i < 6; i++) {
      if (e.code === `Digit${i + 1}` && inventory.includes(weaponKeys[i]!)) {
        ...
        spawnFloatingText(
          player.x + player.w / 2,
          player.y - 20,
          WEAPONS[weaponKeys[i]!].name,
          WEAPONS[weaponKeys[i]!].color,
          14,
        )
      }
    }
  ...
  if (e.code === 'KeyF' && gameState.value === 'playing' && !showInventory.value) {
    ...
    if (hp && playerHp.value < playerMaxHp.value) {
      hp.count--
      playerHp.value = Math.min(playerHp.value + HP_POTION_HEAL, playerMaxHp.value)
      spawnFloatingText(
        player.x + player.w / 2,
        player.y - 20,
        `+${HP_POTION_HEAL} HP`,
  ...
  if (e.code === 'KeyG' && gameState.value === 'playing' && !showInventory.value) {
    ...
    if (mp && playerMp.value < playerMaxMp.value) {
      mp.count--
      playerMp.value = Math.min(playerMp.value + MP_POTION_HEAL, playerMaxMp.value)
      spawnFloatingText(
        player.x + player.w / 2,
        player.y - 20,
        `+${MP_POTION_HEAL} MP`,
  ...
  if (showInventory.value && invTab === 1 && gameState.value === 'playing') {
    if (e.code === 'ArrowLeft' || e.code === 'KeyA')
      equipSelectedSlot = Math.max(0, equipSelectedSlot - 1)
    if (e.code === 'ArrowRight' || e.code === 'KeyD')
      equipSelectedSlot = Math.min(5, equipSelectedSlot + 1)
  }
  // NPC blacksmith keyboard controls
  if (activeNPC?.type === 'blacksmith') {
    if (e.code === 'Escape') {
      upgradeSelectedEquip = null
      upgradeResult = 'none'
    }
    if (e.code === 'KeyD') {
      upgradeUseRefineDust = !upgradeUseRefineDust && materialInventory.refine_dust > 0
    }
    if (e.code === 'KeyF') {
      upgradeUseProtection = !upgradeUseProtection && materialInventory.protection_scroll > 0
    }
    if (!upgradeSelectedEquip) {
      ...
      if (e.code === 'ArrowDown')
        upgradeListIdx = Math.min(allUpgradeable.length - 1, upgradeListIdx + 1)
    ...
    if (e.code === 'Enter') {
      ...
      if (item && playerGold.value >= item.price) {
        ...
        spawnFloatingText(
          player.x + player.w / 2,
          player.y - 20,
          `${MATERIALS[item.mat].icon} +1`,
          MATERIALS[item.mat].color,
          14,
        )
      } else {
        sfxPlayerHurt()
    ...
    if (!upgradeSelectedEquip) {
      ...
      for (let i = startI; i < Math.min(allItems.length, startI + maxVisible); i++) {
        ...
        if (mx >= px + 12 && mx <= px + panelW - 12 && my >= iy && my <= iy + 34) {
          upgradeSelectedEquip = allItems[i]!
          upgradeListIdx = i
          sfxItem()
          return
        }
      ...
      if (mx >= sx && mx <= sx + slotSize && my >= sy && my <= sy + slotSize) {
        if (inventory.includes(allWeapons[i]!)) {
          ...
          spawnFloatingText(
            player.x + player.w / 2,
            player.y - 20,
            WEAPONS[allWeapons[i]!].name,
            WEAPONS[allWeapons[i]!].color,
            14,
          )
        }
        return
    ...
    for (let i = 0; i < Math.min(equipInventory.length, maxShow); i++) {
          ...
          if (materials.length >= 2) {
            ...
            if (baseIdx >= 0) {
              ...
              spawnFloatingText(
                player.x + player.w / 2,
                player.y - 20,
                `⬆ ${upgraded.name} Lv.${upgraded.level}!`,
                '#fbbf24',
                16,
              )
            }
      ...
      if (mx >= eqBtnX && mx <= eqBtnX + 30 && my >= eqBtnY && my <= eqBtnY + 14) {
        ...
        spawnFloatingText(
          player.x + player.w / 2,
          player.y - 20,
          `${eq.icon} ${eq.name} trang bị!`,
          RARITY_COLORS[eq.rarity],
          14,
        )
        return
      }
...
function resetPlayer() {
  Object.assign(player, {
    ...
    attackTimer: 0,
    attackCooldown: 0,
    invincible: PLAYER_SPAWN_INVINCIBLE,
    state: 'idle',
    facing: 1,
    baseAtk: PLAYER_BASE_ATK,
    atkBoost: 0,
  ...
  playerMaxHp.value = PLAYER_MAX_HP
  playerMp.value = PLAYER_MAX_MP
  playerMaxMp.value = PLAYER_MAX_MP
  playerExp.value = 0
  level.value = 1
  score.value = 0
  combo.value = 0
  ...
  shopSelectedIdx = 0
  upgradeSelectedEquip = null
  upgradeUseRefineDust = false
  upgradeUseProtection = false
  upgradeResult = 'none'
  upgradeAnim = 0
}
...
function performUpgrade() {
  ...
  if (upgradeUseProtection && materialInventory.protection_scroll > 0)
    materialInventory.protection_scroll--
  ...
  if (success) {
    ...
    if (eq.stats.speed)
      eq.stats.speed = Math.round((eq.stats.speed / prevBoost) * boost * 100) / 100
    if (eq.stats.critChance)
      eq.stats.critChance = Math.round((eq.stats.critChance / prevBoost) * boost)
    eq.level = newLevel
...
function tryUseSkill(skill: SkillType) {
  ...
  if (skill === 'dash') {
    player.dashing = true
    player.dashTimer = DASH_TIMER
    player.vx = player.facing * DASH_SPEED
    player.invincible = DASH_INVINCIBLE_FRAMES
    ...
    projectiles.push({
      ...
      w: 40,
      h: player.h,
      damage: DASH_SLASH_DAMAGE + player.baseAtk,
      life: DASH_SLASH_LIFE,
      type: 'dash_slash',
      color: '#38bdf8',
      rotation: 0,
      piercing: true,
...
function spawnMonster() {
  ...
  for (const zone of mapCfg.zones) {
    ...
    // Đếm quái trong zone
    const zoneMonsters = monsters.filter(
      (m) => !m.dead && m.x >= zone.xStart - 100 && m.x <= zone.xEnd + 100,
    )
...
function drawPixelChar(
  ...
  const headbandColor = player.equipment.head
    ? RARITY_COLORS[player.equipment.head.rarity]
    : '#ef4444'
  ...
  if (state === 'run') {
    ctx.fillRect(-11 + tailWave * 1.5, 6, 3, 1)
  }
  ...
  if (state === 'jump' && !isDoubleJump) {
    ...
    ctx.fillRect(7, h - 12, 6, 6)
    ctx.fillRect(w - 13, h - 12, 6, 6)
    ctx.fillStyle = '#3b2a1a'
    ctx.fillRect(7, h - 7, 7, 3)
    ctx.fillRect(w - 14, h - 7, 7, 3)
  } else if (isDoubleJump) {
    // Tucked spin legs
    ctx.fillStyle = '#1e3a5f'
    ctx.fillRect(6, h - 14, 6, 4)
    ctx.fillRect(w - 12, h - 14, 6, 4)
    ctx.fillStyle = '#3b2a1a'
    ctx.fillRect(6, h - 10, 6, 2)
    ctx.fillRect(w - 12, h - 10, 6, 2)
  } else {
    ...
    ctx.fillStyle = player.equipment.boots
      ? RARITY_COLORS[player.equipment.boots.rarity]
      : '#3b2a1a'
  ...
  if (state === 'attack') {
    ...
    if (player.weapon === 'sword') {
      ...
      ctx.fillStyle = '#92400e'
      ctx.fillRect(0, -1, 28, 3) // handle
      ctx.fillStyle = '#a78bfa'
      ctx.fillRect(22, -6, 10, 12) // blade head
      ...
      ctx.lineWidth = 2
      ctx.beginPath()
      ctx.arc(10, 0, 14, -1.2, 1.2)
      ctx.stroke()
      ctx.fillStyle = '#fb923c'
      ctx.fillRect(10, -10, 1, 20) // arrow
      ctx.fillStyle = '#ef4444'
      ctx.fillRect(10, -12, 3, 4) // arrowhead
      ...
      ctx.fillStyle = '#92400e'
      ctx.fillRect(0, -1, 24, 3) // handle
      ctx.fillStyle = '#fbbf24'
      ctx.fillRect(18, -8, 14, 16) // head
    ...
    if (player.weapon === 'sword' || player.weapon === 'dual_swords') {
      ...
      if (player.weapon === 'dual_swords') {
        ctx.fillStyle = '#60a5fa'
        ctx.fillRect(w - 7, -4, 2, 18)
      }
    } else if (player.weapon === 'axe') {
      ctx.fillStyle = '#92400e'
      ctx.fillRect(w - 4, -4, 2, 20)
      ctx.fillStyle = '#a78bfa'
      ctx.fillRect(w - 8, -6, 8, 6)
    } else if (player.weapon === 'bow') {
      ctx.strokeStyle = '#fb923c'
      ctx.lineWidth = 1.5
      ctx.beginPath()
      ctx.arc(w - 2, 6, 10, -1, 1)
      ctx.stroke()
    } else if (player.weapon === 'shuriken') {
      ctx.fillStyle = '#5eead4'
      ctx.fillRect(w - 6, 0, 4, 4)
      ctx.fillRect(w - 4, -2, 4, 4)
    } else if (player.weapon === 'hammer') {
      ctx.fillStyle = '#92400e'
      ctx.fillRect(w - 3, -2, 2, 16)
      ctx.fillStyle = '#fbbf24'
      ctx.fillRect(w - 7, -6, 10, 6)
    }
  }
  ...
  if (player.dashing) {
    ...
    ctx.lineWidth = 3
    ctx.beginPath()
    ctx.ellipse(w / 2, h / 2, w / 2 + 8, h / 2 + 6, 0, 0, Math.PI * 2)
    ctx.stroke()
  }
...
function drawMonster(m: Monster) {
  ...
  if (m.type === 'slime') {
    ...
    if (biome === 'desert') {
      ...
      ctx.beginPath()
      ctx.ellipse(m.w / 2, m.h - 4, (m.w / 2) * squash + 2, m.h / 2 / squash + 1, 0, 0, Math.PI * 2)
      ctx.fill()
    } else if (biome === 'volcano') {
      ctx.fillStyle = 'rgba(251,146,60,0.4)'
      ctx.beginPath()
      ctx.ellipse(m.w / 2, m.h - 4, (m.w / 2) * squash + 3, m.h / 2 / squash + 2, 0, 0, Math.PI * 2)
      ctx.fill()
    ...
    ctx.fillRect(m.w - 13, 10, 3, 3)
    const lOff = Math.sin(m.animFrame * 0.2) * 3
    ctx.fillStyle = m.color
    ctx.fillRect(8, 32, 4, 6 + lOff)
    ctx.fillRect(m.w - 12, 32, 4, 6 - lOff)
    const weaponColor =
      biome === 'desert'
        ? '#b45309'
        : biome === 'ice'
    ...
    if (biome === 'desert') {
      ctx.fillStyle = '#f59e0b'
      ctx.fillRect(5, 3, m.w - 10, 5)
      ctx.fillRect(8, 0, 4, 5)
    } else if (biome === 'ice') {
      ctx.fillStyle = '#bae6fd'
      ctx.fillRect(6, 18, m.w - 12, 3)
      ctx.fillStyle = 'rgba(147,197,253,0.5)'
      ctx.fillRect(4, 8, 2, 8)
      ctx.fillRect(m.w - 6, 8, 2, 8)
    } else if (biome === 'volcano') {
      ctx.fillStyle = '#f97316'
      ctx.fillRect(10, 12, 2, 6)
      ctx.fillRect(m.w - 13, 14, 2, 4)
      const ft = Date.now() * 0.008
      ctx.fillStyle = '#ef4444'
      ctx.fillRect(m.w / 2 + Math.sin(ft) * 5, -2 + Math.cos(ft * 1.5) * 2, 2, 2)
    }
  } else if (m.type === 'demon') {
    ctx.fillStyle = m.color
    ctx.fillRect(4, 8, m.w - 8, m.h - 12)
    const hornColor = biome === 'ice' ? '#3b82f6' : biome === 'volcano' ? '#1a1a2e' : '#7f1d1d'
    ctx.fillStyle = hornColor
    ctx.fillRect(4, 0, 4, 10)
    ctx.fillRect(m.w - 8, 0, 4, 10)
    if (biome === 'volcano') {
      ctx.fillRect(2, -2, 3, 6)
      ctx.fillRect(m.w - 5, -2, 3, 6)
    }
    const demonEyeColor = biome === 'ice' ? '#93c5fd' : '#fbbf24'
    ctx.fillStyle = demonEyeColor
    ctx.fillRect(10, 14, 4, 4)
    ctx.fillRect(m.w - 14, 14, 4, 4)
    const wingColor =
      biome === 'ice' ? '99,102,241' : biome === 'volcano' ? '239,68,68' : '248,113,113'
    const wingAlpha = biome === 'volcano' ? '0.7' : '0.5'
    ctx.fillStyle = `rgba(${wingColor},${wingAlpha})`
    ctx.fillRect(-8, 12, 12, 16)
    ctx.fillRect(m.w - 4, 12, 12, 16)
    const feetColor = biome === 'ice' ? '#312e81' : biome === 'volcano' ? '#450a0a' : '#991b1b'
    ctx.fillStyle = feetColor
    ctx.fillRect(8, m.h - 6, 6, 6)
    ctx.fillRect(m.w - 14, m.h - 6, 6, 6)
    if (biome === 'ice') {
      ctx.strokeStyle = 'rgba(147,197,253,0.3)'
      ctx.lineWidth = 1
      ctx.beginPath()
      ctx.ellipse(m.w / 2, m.h / 2, m.w / 2 + 4, m.h / 2 + 2, 0, 0, Math.PI * 2)
      ctx.stroke()
    } else if (biome === 'volcano') {
      ...
      for (let i = 0; i < 3; i++) {
        ctx.fillRect(
          m.w / 2 + Math.sin(ft + i * 2) * (m.w / 2 + 6),
          m.h / 3 + Math.cos(ft + i * 2.5) * 8,
          2,
          3,
        )
      }
    ...
    ctx.fillStyle = m.color
    ctx.fillRect(6, 10, m.w - 12, m.h - 16)
    const crownColor =
      biome === 'desert'
        ? '#f59e0b'
        : biome === 'ice'
    ...
    ctx.fillRect(10, 0, m.w - 20, 6)
    ctx.fillRect(10, -4, 4, 6)
    ctx.fillRect(m.w / 2 - 2, -6, 4, 8)
    ctx.fillRect(m.w - 14, -4, 4, 6)
    ctx.fillStyle = '#1a1a2e'
    ctx.fillRect(14, 18, 6, 6)
    ctx.fillRect(m.w - 20, 18, 6, 6)
    ctx.fillStyle = '#ef4444'
    ctx.fillRect(16, 30, m.w - 32, 4)
    const auraColor =
      biome === 'desert'
        ? 'rgba(245,158,11,0.4)'
        : biome === 'ice'
    ...
    ctx.ellipse(
      m.w / 2,
      m.h / 2,
      m.w / 2 + 6 + Math.sin(Date.now() * 0.005) * 4,
      m.h / 2 + 4,
      0,
      0,
      Math.PI * 2,
    ...
    if (biome === 'desert') {
      ...
      ctx.fillRect(m.w - 8, 4, 4, 14)
      ctx.fillStyle = '#b45309'
      ctx.fillRect(8, 8, m.w - 16, 2)
    } else if (biome === 'ice') {
      ctx.fillStyle = '#e0f2fe'
      ctx.fillRect(m.w / 2 - 1, -10, 2, 6)
      ctx.fillRect(m.w / 2 - 6, -7, 2, 5)
      ctx.fillRect(m.w / 2 + 4, -7, 2, 5)
    } else if (biome === 'volcano') {
      ctx.fillStyle = '#fb923c'
      ctx.fillRect(10, 14, 3, m.h - 24)
      ctx.fillRect(m.w - 13, 18, 3, m.h - 28)
      const ft = Date.now() * 0.005
      for (let i = 0; i < 5; i++) {
        ctx.fillStyle = i % 2 === 0 ? '#fbbf24' : '#ef4444'
        ctx.fillRect(
          m.w / 2 + Math.sin(ft + i * 1.3) * (m.w / 2 + 10),
          m.h / 3 + Math.cos(ft + i * 1.7) * 12,
          2,
          3,
        )
      }
    ...
    ctx.fillStyle = m.color
    ctx.beginPath()
    ctx.ellipse(m.w / 2, m.h / 2 + 2, 6, 5, 0, 0, Math.PI * 2)
    ctx.fill()
    const wingAngle = Math.sin(m.animFrame * 0.3) * 0.6
    const wingSize = biome === 'volcano' ? 14 : biome === 'ice' ? 13 : 12
    ctx.save()
    ctx.translate(m.w / 2 - 5, m.h / 2)
    ctx.rotate(-wingAngle)
    ctx.fillStyle = m.color
    ctx.fillRect(-wingSize, -3, wingSize, 6)
    if (biome === 'desert') {
      ctx.fillStyle = 'rgba(146,64,14,0.4)'
      ctx.fillRect(-wingSize + 2, -1, wingSize - 4, 2)
    } else if (biome === 'volcano') {
      ctx.fillStyle = 'rgba(251,146,60,0.5)'
      ctx.fillRect(-wingSize + 1, -2, wingSize - 2, 4)
    }
    ctx.restore()
    ctx.save()
    ctx.translate(m.w / 2 + 5, m.h / 2)
    ctx.rotate(wingAngle)
    ctx.fillStyle = m.color
    ctx.fillRect(0, -3, wingSize, 6)
    if (biome === 'desert') {
      ctx.fillStyle = 'rgba(146,64,14,0.4)'
      ctx.fillRect(2, -1, wingSize - 4, 2)
    } else if (biome === 'volcano') {
      ctx.fillStyle = 'rgba(251,146,60,0.5)'
      ctx.fillRect(1, -2, wingSize - 2, 4)
    }
    ctx.restore()
    const batEyeColor =
      biome === 'desert'
    ...
    ctx.fillRect(m.w / 2 - 4, m.h / 2 - 2, 2, 2)
    ctx.fillRect(m.w / 2 + 2, m.h / 2 - 2, 2, 2)
    ctx.fillStyle = '#fff'
    ctx.fillRect(m.w / 2 - 2, m.h / 2 + 4, 1, 2)
    ctx.fillRect(m.w / 2 + 1, m.h / 2 + 4, 1, 2)
    if (biome === 'ice') {
      ctx.fillStyle = 'rgba(186,230,253,0.3)'
      ctx.fillRect(m.w / 2 - 8, m.h / 2 + 5, 16, 2)
    }
    ...
    ctx.fill()
    const ghostEyeColor =
      biome === 'desert' ? '#f59e0b' : biome === 'volcano' ? '#ef4444' : '#1a1a2e'
    ctx.fillStyle = ghostEyeColor
    ctx.beginPath()
    ctx.ellipse(m.w / 3, m.h / 3 + 2 + wave * 0.3, 3, 4, 0, 0, Math.PI * 2)
    ctx.fill()
    ctx.beginPath()
    ctx.ellipse((m.w * 2) / 3, m.h / 3 + 2 + wave * 0.3, 3, 4, 0, 0, Math.PI * 2)
    ctx.fill()
    ctx.beginPath()
    ctx.ellipse(m.w / 2, m.h / 2 + wave * 0.3, 4, 3, 0, 0, Math.PI)
    ctx.fill()
    ctx.globalAlpha = 1
    if (biome === 'ice') {
      ctx.fillStyle = '#e0f2fe'
  ...
  // === DESERT MONSTERS ===
    ...
    ctx.fillStyle = m.color
    ctx.beginPath()
    ctx.ellipse(m.w / 2, m.h - 4, m.w / 2 - 2, 6, 0, 0, Math.PI * 2)
    ctx.fill()
    // Claws
    const clawAnim = Math.sin(m.animFrame * 0.2) * 3
    ctx.fillStyle = '#b45309'
    ctx.fillRect(2, m.h / 2 - 6 + clawAnim, 6, 4)
    ctx.fillRect(0, m.h / 2 - 8 + clawAnim, 3, 3)
    ctx.fillRect(m.w - 8, m.h / 2 - 6 - clawAnim, 6, 4)
    ctx.fillRect(m.w - 3, m.h / 2 - 8 - clawAnim, 3, 3)
    // Tail (curved up)
    ...
    ctx.fillStyle = '#ef4444'
    ctx.fillRect(m.w / 2 - 2 + tailSway, m.h / 2 - 16, 3, 3) // stinger
    // Eyes
    ctx.fillStyle = '#1a1a2e'
    ctx.fillRect(m.w / 2 - 5, m.h - 8, 2, 2)
    ctx.fillRect(m.w / 2 + 3, m.h - 8, 2, 2)
    // Legs
    ctx.fillStyle = '#92400e'
    for (let i = 0; i < 3; i++) {
    ...
    ctx.fillRect(8, 4, m.w - 16, 10)
    // Glowing eyes
    const eyeGlow = 0.6 + Math.sin(Date.now() * 0.005) * 0.4
    ctx.fillStyle = `rgba(251,191,36,${eyeGlow})`
    ctx.fillRect(10, 8, 3, 3)
    ctx.fillRect(m.w - 13, 8, 3, 3)
    // Dangling bandages
    const dangle = Math.sin(Date.now() * 0.004 + m.x) * 3
    ctx.fillStyle = '#a8a29e'
    ctx.fillRect(4, 12, 3, 10 + dangle)
    ctx.fillRect(m.w - 7, 16, 3, 8 - dangle)
    // Arms reaching forward
    const armSwing = Math.sin(m.animFrame * 0.15) * 4
    ctx.fillStyle = '#d6d3d1'
    ctx.fillRect(m.w - 2, 14 + armSwing, 8, 4)
    ctx.fillRect(m.w + 4, 12 + armSwing, 3, 3)
    // Legs shuffle
    const lOff = Math.sin(m.animFrame * 0.12) * 3
    ctx.fillStyle = m.color
    ctx.fillRect(8, m.h - 6, 5, 6 + lOff)
    ctx.fillRect(m.w - 13, m.h - 6, 5, 6 - lOff)
    ...
    ctx.ellipse(
      seg / 2,
      m.h / 2 + Math.sin(m.animFrame * 0.2) * 4,
      seg / 2 + 3,
      m.h / 2,
      0,
      0,
    ...
    ctx.fillStyle = '#ef4444'
    ctx.beginPath()
    ctx.arc(2, m.h / 2 + Math.sin(m.animFrame * 0.2) * 4, 4, -0.8, 0.8)
    ctx.fill()
    // Eyes
    ctx.fillStyle = '#fbbf24'
    ctx.fillRect(4, m.h / 2 - 5 + Math.sin(m.animFrame * 0.2) * 4, 2, 2)
  ...
  // === ICE MONSTERS ===
    ...
    ctx.fillRect(6, 2, m.w - 12, 14)
    ctx.fillStyle = '#dbeafe'
    ctx.fillRect(8, 0, 4, 6)
    ctx.fillRect(m.w - 12, 0, 4, 6)
    // Crystal crown
    ctx.fillStyle = '#e0f2fe'
    ctx.fillRect(m.w / 2 - 2, -6, 4, 8)
    ctx.fillRect(m.w / 2 - 6, -3, 3, 5)
    ctx.fillRect(m.w / 2 + 3, -3, 3, 5)
    // Eyes (glowing blue)
    const iceGlow = 0.7 + Math.sin(Date.now() * 0.004) * 0.3
    ctx.fillStyle = `rgba(59,130,246,${iceGlow})`
    ctx.fillRect(10, 8, 4, 4)
    ctx.fillRect(m.w - 14, 8, 4, 4)
    // Ice texture lines
    ctx.fillStyle = 'rgba(219,234,254,0.4)'
    ctx.fillRect(8, 18, m.w - 16, 1)
    ctx.fillRect(10, 26, m.w - 20, 1)
    ctx.fillRect(8, 34, m.w - 16, 1)
    // Arms (blocky)
    const armSwing2 = Math.sin(m.animFrame * 0.1) * 3
    ctx.fillStyle = '#93c5fd'
    ctx.fillRect(-4, 16 + armSwing2, 8, 6)
    ctx.fillRect(m.w - 4, 16 - armSwing2, 8, 6)
    ctx.fillStyle = '#60a5fa'
    ctx.fillRect(-2, 20 + armSwing2, 6, 8)
    ctx.fillRect(m.w - 4, 20 - armSwing2, 6, 8)
    // Legs
    ctx.fillStyle = m.color
    ctx.fillRect(8, m.h - 6, 6, 6)
    ctx.fillRect(m.w - 14, m.h - 6, 6, 6)
    ...
    for (let i = 0; i < 3; i++) {
      ctx.fillRect(
        m.w / 2 + Math.sin(ift + i * 2) * (m.w / 2 + 4),
        (Math.cos(ift + i * 2) * m.h) / 2 + m.h / 3,
        2,
        2,
      )
    }
    ...
    ctx.fillRect(m.w - 8, -2, 4, 8)
    // Angry eyes
    ctx.fillStyle = '#ef4444'
    ctx.fillRect(10, 6, 4, 3)
    ctx.fillRect(m.w - 14, 6, 4, 3)
    ctx.fillStyle = '#1a1a2e'
    ctx.fillRect(11, 7, 2, 2)
    ctx.fillRect(m.w - 13, 7, 2, 2)
    // Mouth/roar
    const roar = Math.sin(m.animFrame * 0.15) > 0.5
    if (roar) {
      ctx.fillStyle = '#1a1a2e'
      ctx.fillRect(12, 12, m.w - 24, 3)
      ctx.fillStyle = '#fff'
      ctx.fillRect(13, 12, 2, 2)
      ctx.fillRect(m.w - 15, 12, 2, 2) // fangs
    }
    // Big arms
    const yArmSwing = Math.sin(m.animFrame * 0.15) * 5
    ctx.fillStyle = '#d1d5db'
    ctx.fillRect(-4, 14 + yArmSwing, 8, 12)
    ctx.fillRect(m.w - 4, 14 - yArmSwing, 8, 12)
    ctx.fillStyle = '#e2e8f0'
    ctx.fillRect(-2, 24 + yArmSwing, 6, 5)
    ctx.fillRect(m.w - 4, 24 - yArmSwing, 6, 5) // fists
    // Legs
    const yLeg = Math.sin(m.animFrame * 0.15) * 3
    ctx.fillStyle = '#d1d5db'
    ctx.fillRect(8, m.h - 6, 7, 6 + yLeg)
    ctx.fillRect(m.w - 15, m.h - 6, 7, 6 - yLeg)
    // Snow dust
    if (m.state === 'chase') {
    ...
    ctx.beginPath()
    ctx.ellipse(m.w / 2, m.h / 2 + wave, m.w / 2 + 4, m.h / 2 + 2, 0, 0, Math.PI * 2)
    ctx.fill()
    // Inner core
    ctx.fillStyle = m.color
    ctx.beginPath()
    ctx.ellipse(m.w / 2, m.h / 2 + wave, m.w / 3, m.h / 3, 0, 0, Math.PI * 2)
    ctx.fill()
    // Ice crystal core
    ctx.fillStyle = '#e0f2fe'
    ctx.fillRect(m.w / 2 - 2, m.h / 3 + wave - 4, 4, 8)
    ctx.fillRect(m.w / 2 - 4, m.h / 3 + wave - 1, 8, 3)
    // Eyes
    ctx.fillStyle = '#3b82f6'
    ctx.fillRect(m.w / 3, m.h / 3 + wave, 3, 3)
    ctx.fillRect((m.w * 2) / 3 - 2, m.h / 3 + wave, 3, 3)
    // Floating ice shards
  ...
  // === VOLCANO MONSTERS ===
    ...
    for (let i = 0; i < segments2; i++) {
      ...
      ctx.beginPath()
      ctx.ellipse(segW * i + segW / 2, m.h / 2 + sy2, segW / 2 + 1, m.h / 2 - 1, 0, 0, Math.PI * 2)
      ctx.fill()
    }
    // Head
    ...
    ctx.ellipse(
      segW / 2,
      m.h / 2 + Math.sin(m.animFrame * 0.25) * 3,
      segW / 2 + 3,
      m.h / 2 + 1,
      0,
      0,
    ...
    ctx.fillStyle = m.color
    ctx.beginPath()
    ctx.ellipse(m.w / 2, m.h / 2 + 4, m.w / 2 - 2, m.h / 2 - 4, 0, 0, Math.PI * 2)
    ctx.fill()
    // Inner flame
    ctx.fillStyle = '#fbbf24'
    ctx.beginPath()
    ctx.ellipse(m.w / 2, m.h / 2 + 2, m.w / 3, m.h / 3, 0, 0, Math.PI * 2)
    ctx.fill()
    // Core
    ctx.fillStyle = '#fef3c7'
    ctx.beginPath()
    ctx.ellipse(m.w / 2, m.h / 2, m.w / 5, m.h / 5, 0, 0, Math.PI * 2)
    ctx.fill()
    // Flame tips on top
    for (let i = 0; i < 4; i++) {
    ...
    // Eyes (dark)
    ctx.fillStyle = '#1a1a2e'
    ctx.fillRect(m.w / 3, m.h / 3 + 2, 3, 4)
    ctx.fillRect((m.w * 2) / 3 - 2, m.h / 3 + 2, 3, 4)
    // Ember particles
    ctx.fillStyle = '#fbbf24'
    for (let i = 0; i < 4; i++) {
    ...
    ctx.fillRect(m.w - 14, 20, 5, 5)
    ctx.fillRect(12, 30, 7, 5)
    // Head
    ctx.fillStyle = '#292524'
    ctx.fillRect(8, 2, m.w - 16, 12)
    // Lava cracks (glowing)
    ...
    ctx.fillRect(6, 20, 2, m.h - 28)
    ctx.fillRect(m.w - 8, 18, 2, m.h - 26) // body cracks
    ctx.fillRect(12, 28, m.w - 24, 2)
    ctx.fillRect(10, 38, m.w - 20, 2) // horizontal cracks
    // Lava eyes
    ctx.fillStyle = `rgba(239,68,68,${lavaGlow})`
    ctx.fillRect(12, 5, 5, 4)
    ctx.fillRect(m.w - 17, 5, 5, 4)
    // Fists
    ...
    ctx.fillRect(-4, 18 + mgArmSwing, 6, 3)
    ctx.fillRect(m.w - 2, 18 - mgArmSwing, 6, 3)
    // Legs
    ctx.fillStyle = '#292524'
    ctx.fillRect(8, m.h - 6, 8, 6)
    ctx.fillRect(m.w - 16, m.h - 6, 8, 6)
    // Molten drips
    ctx.fillStyle = '#fb923c'
...
function drawBackground() {
  ...
  // Moon
  ctx.fillStyle = biome.moonColor
  ctx.beginPath()
  ctx.arc(W - 120, 80, 35, 0, Math.PI * 2)
  ctx.fill()
  ctx.fillStyle = biome.skyColors[0]!
  ctx.beginPath()
  ctx.arc(W - 108, 72, 30, 0, Math.PI * 2)
  ctx.fill()
  ...
  for (const m of bgMountains) {
    ...
    ctx.beginPath()
    ctx.moveTo(mx, H - 48)
    ctx.lineTo(mx + m.w / 2, H - 48 - m.h)
    ctx.lineTo(mx + m.w, H - 48)
    ctx.fill()
  }
  // Trees
  ...
  for (const t of bgTrees) {
    ...
    ctx.fillStyle = biome.treeTrunk
    ctx.fillRect(tx + t.w / 2 - 3, H - 48 - t.h * 0.4, 6, t.h * 0.4)
    ctx.fillStyle = biome.treeColor
    ctx.beginPath()
    ctx.moveTo(tx, H - 48 - t.h * 0.3)
    ctx.lineTo(tx + t.w / 2, H - 48 - t.h)
    ctx.lineTo(tx + t.w, H - 48 - t.h * 0.3)
    ctx.fill()
  }
  ...
  if (getBiomeForLevel(level.value) === 'ice') {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      ctx.fillStyle = `rgba(239,68,68,${0.3 + Math.sin(Date.now() * 0.005 + i) * 0.2})`
...
function drawPlatforms() {
  ...
  for (const p of platforms) {
    ...
    if (p.type === 'ground') {
      ...
      ctx.fillRect(px, py, Math.min(p.w, W + 20), p.h)
      ctx.fillStyle = biome.grassColor
      ctx.fillRect(px, py, Math.min(p.w, W + 20), 4)
      ctx.fillStyle = biome.grassColor2
      ctx.fillRect(px, py + 4, Math.min(p.w, W + 20), 2)
    } else {
...
function updatePlayer() {
  ...
  if ((keys['KeyZ'] || keys['KeyJ']) && player.attackCooldown <= 0 && !player.attacking) {
    ...
    player.attackCooldown = atkCooldown
  ...
  if (player.attacking) {
    ...
    const atkBox = {
      ...
      y: player.y - ATTACK_HITBOX_PAD_Y,
      w: range,
      h: player.h + ATTACK_HITBOX_PAD_H,
    }
    for (const m of monsters) {
      ...
      if (rectCollide(atkBox, m)) {
        ...
        let dmg = Math.floor(
          (wCfg.damage +
            player.baseAtk +
            level.value * DAMAGE_LEVEL_SCALE +
            player.atkBoost * DAMAGE_ATK_BOOST_MULT +
            eqBonus.atk +
            Math.random() * DAMAGE_VARIANCE) *
            comboMult,
        )
        ...
        m.hp -= dmg
        m.hurtTimer = HURT_TIMER_FRAMES
        m.vx = player.facing * MONSTER_HIT_KNOCKBACK
        m.vy = m.flying ? -2 : -3
        ...
        spawnFloatingText(
          ...
          m.y - 10,
          isCrit ? `💥 ${dmg}` : `-${dmg}`,
          isCrit ? '#ff6b6b' : '#fbbf24',
          isCrit ? 18 : 14,
        )
        // === SCREEN SHAKE (always) + HIT-STOP (only if not already frozen) ===
        screenShake.intensity = Math.max(
          screenShake.intensity,
          isCrit ? SHAKE_INTENSITY_CRIT : SHAKE_INTENSITY_HIT,
        )
        ...
        if (combo.value > 1)
          spawnFloatingText(
            player.x + player.w / 2,
            player.y - 30,
            `${combo.value} HIT! x${comboMult.toFixed(1)}`,
            '#f97316',
            12 + Math.min(combo.value, 10),
          )
        if (m.hp <= 0) {
          m.dead = true
          killCount.value++
          ...
          if (Math.random() < WEAPON_DROP_CHANCE)
            weaponDrops.push(createWeaponDrop(m.x + m.w / 2 - 10, m.y))
          // Equipment drop
          ...
          if (Math.random() < CONSUMABLE_DROP_CHANCE) {
            ...
            spawnFloatingText(
              m.x + m.w / 2,
              m.y,
              cType === 'hp_potion' ? '❤️ +1 HP Potion' : '💙 +1 MP Potion',
              cType === 'hp_potion' ? '#ef4444' : '#3b82f6',
              12,
            )
          }
          // === GOLD DROP ===
          ...
          if (matDrop) {
            ...
            spawnFloatingText(
              m.x + m.w / 2,
              m.y - 45,
              `${MATERIALS[matDrop].icon} +1`,
              MATERIALS[matDrop].color,
              13,
            )
          }
          // === WING DROP ===
    ...
    if (rectCollide(player, d)) {
      if (!inventory.includes(d.weapon)) {
        ...
        spawnFloatingText(
          d.x + d.w / 2,
          d.y - 20,
          `${WEAPONS[d.weapon].icon} ${WEAPONS[d.weapon].name}!`,
          WEAPONS[d.weapon].color,
          16,
        )
        sfxChestOpen()
      } else {
...
function updateMonsters() {
  ...
  for (let i = monsters.length - 1; i >= 0; i--) {
    ...
    // === BOSS MULTI-PHASE BEHAVIOR ===
      ...
      const phaseSpeedMult =
        phase === 3 ? BOSS_PHASE3_SPEED : phase === 2 ? BOSS_PHASE2_SPEED : BOSS_PHASE1_SPEED
      const phaseCooldown =
        phase === 3
          ? BOSS_PHASE3_COOLDOWN
          : phase === 2
      ...
      if (m.attackTimer > 0) {
        ...
        m.attackTimer = MONSTER_ATTACK_TIMER
        m.attackCooldown = phaseCooldown
        ...
        if (!isAdmin) {
          ...
          player.vx = -m.facing * KNOCKBACK_VX
        ...
        if (passives.includes('thorns') && !m.dead) {
          ...
          m.hp -= thornsDmg
          m.hurtTimer = MINOR_HURT_TIMER
          spawnFloatingText(m.x + m.w / 2, m.y - 10, `🌵 ${thornsDmg}`, '#4ade80', 12)
        }
        if (!isAdmin && playerHp.value <= 0) {
          playerHp.value = 0
        ...
        m.state = 'chase'
        m.vx = m.facing * m.speed * phaseSpeedMult
  ...
  switch (item) {
    case 'hp_potion':
      ...
      spawnFloatingText(
        player.x + player.w / 2,
        player.y - 20,
        `+${HP_POTION_HEAL_CHEST} HP`,
        '#ef4444',
        16,
      )
      break
    case 'atk_boost':
      player.atkBoost = ITEM_DURATIONS.atk_boost / 60
      break
    case 'speed_boost':
      player.speedBoost = ITEM_DURATIONS.speed_boost / 60
      break
    case 'shield':
      player.shield = ITEM_DURATIONS.shield / 60
      break
    case 'exp_gem':
...
function levelUp() {
  level.value++
  playerExp.value = 0
  playerExpMax.value = Math.floor(playerExpMax.value * EXP_GROWTH)
  playerMaxHp.value += LEVEL_HP_BONUS
  playerHp.value = Math.min(playerHp.value + LEVEL_HP_HEAL, playerMaxHp.value)
  playerMaxMp.value += LEVEL_MP_BONUS
  playerMp.value = Math.min(playerMp.value + LEVEL_MP_HEAL, playerMaxMp.value)
  player.speed += LEVEL_SPEED_BONUS
  player.baseAtk += LEVEL_ATK_BONUS
  sfxLevelUp()
...
function autoSave() {
  ...
  try {
    const saveData = {
      ...
      score: score.value,
      hp: playerHp.value,
      maxHp: playerMaxHp.value,
      mp: playerMp.value,
      maxMp: playerMaxMp.value,
      exp: playerExp.value,
      expMax: playerExpMax.value,
      kills: killCount.value,
      ngPlus: newGamePlusLevel,
      equipment: player.equipment,
      equipInventory: equipInventory.slice(0, 20),
      weaponIdx: player.weapon,
      consumables,
    }
...
function loadSave(): boolean {
  try {
    ...
    playerHp.value = d.hp ?? 100
    playerMaxHp.value = d.maxHp ?? 100
    playerMp.value = d.mp ?? 60
    playerMaxMp.value = d.maxMp ?? 60
    playerExp.value = d.exp ?? 0
    playerExpMax.value = d.expMax ?? 50
    killCount.value = d.kills ?? 0
    newGamePlusLevel = d.ngPlus ?? 0
    if (d.equipment) player.equipment = d.equipment
    if (d.equipInventory) {
      equipInventory.length = 0
      equipInventory.push(...d.equipInventory)
    }
    if (d.consumables) {
      consumables.length = 0
...
function updateFloatingTexts() {
  _updateFloatingTexts(floatingTexts)
}
...
function drawBossHPBar(boss: Monster) {
  ...
  const p2x = barX + barW * BOSS_PHASE2_HP,
    p3x = barX + barW * BOSS_PHASE3_HP
  ...
  hpGrad.addColorStop(1, phase >= 3 ? '#dc2626' : phaseColor)
...
function drawUI() {
  ...
  ctx.fillRect(12, 12, 208, 84)
  ctx.strokeStyle = '#4ade80'
  ctx.lineWidth = 1
  ctx.strokeRect(12, 12, 208, 84)

  // Tên + admin badge
  if (isAdmin) {
    ctx.fillStyle = '#fbbf24'
    ctx.font = 'bold 11px monospace'
    ctx.fillText(`👑 ${playerName.value}`, 18, 28)
    ctx.fillStyle = '#ef4444'
    ctx.font = '8px monospace'
    ctx.fillText('ADMIN', 18, 92)
  } else {
    ctx.fillStyle = '#e2e8f0'
    ctx.font = 'bold 11px monospace'
    ctx.fillText(playerName.value, 18, 28)
  ...
  ctx.fillText(`Lv.${level.value}`, 110, 28)
  ctx.fillStyle = '#94a3b8'
  ctx.font = '9px monospace'
  ctx.fillText(MAP_CONFIGS[currentMapId.value].name, 140, 28)

  // HP
  ctx.fillStyle = '#1a1a2e'
  ctx.fillRect(18, 34, 196, 16)
  ...
  // EXP
  ctx.fillStyle = '#1a1a2e'
  ctx.fillRect(18, 54, 196, 8)
  ctx.fillStyle = '#818cf8'
  ctx.fillRect(18, 54, 196 * (playerExp.value / playerExpMax.value), 8)
  ...
  if (player.atkBoost > 0) {
    ctx.fillStyle = '#f97316'
    ctx.font = '9px monospace'
    ctx.fillText(`⚔${Math.ceil(player.atkBoost)}s`, effX, 80)
    effX += 40
  }
  if (player.speedBoost > 0) {
    ctx.fillStyle = '#3b82f6'
    ctx.fillText(`💨${Math.ceil(player.speedBoost)}s`, effX, 80)
    effX += 40
  }
  if (player.shield > 0) {
    ctx.fillStyle = '#a855f7'
    ctx.fillText(`🛡${Math.ceil(player.shield)}s`, effX, 80)
    effX += 40
  }
  ...
  const wCfg = WEAPONS[player.weapon]
  ctx.fillStyle = 'rgba(15,15,35,0.85)'
  ctx.fillRect(12, 100, 120, 40)
  ctx.strokeStyle = wCfg.color
  ctx.lineWidth = 1
  ctx.strokeRect(12, 100, 120, 40)
  ...
  ctx.fillStyle = 'rgba(15,15,35,0.85)'
  ctx.fillRect(12, 144, 120, 22)
  ctx.strokeStyle = '#64748b'
  ctx.lineWidth = 1
  ctx.strokeRect(12, 144, 120, 22)
  ctx.fillStyle = '#ef4444'
  ctx.font = '9px monospace'
  ...
  const hbW = 28,
    hbGap = 3,
    hbTotal = allW.length * (hbW + hbGap)
  const hbX = (W - hbTotal) / 2,
    hbY = H - 36
  allW.forEach((wt, i) => {
  ...
  // Score panel
  ctx.fillStyle = 'rgba(15,15,35,0.85)'
  ctx.fillRect(W - 170, 12, 158, 48)
  ctx.strokeStyle = '#f97316'
  ctx.strokeRect(W - 170, 12, 158, 48)
  ctx.fillStyle = '#fbbf24'
  ctx.font = 'bold 11px monospace'
  ctx.textAlign = 'right'
  ...
  if (combo.value > 1) {
    ctx.fillStyle = '#f97316'
    ctx.font = `bold ${16 + Math.min(combo.value * 2, 20)}px monospace`
    ctx.textAlign = 'center'
    ctx.fillText(`${combo.value} COMBO!`, W / 2, 40)
    ctx.textAlign = 'left'
  }
  ...
  ctx.font = '7px monospace'
  ctx.fillText(
    '← → Di chuyển | ↑ Nhảy | Z/J Tấn công | Q/E/R Kỹ năng | F/G Thuốc | B Túi đồ',
    16,
    H - 42,
  )
}
...
function drawParticles() {
  if (ctx) _drawParticles(ctx, particles, camera.x)
}
function drawFloatingTexts() {
  if (ctx) _drawFloatingTexts(ctx, floatingTexts, camera.x)
}
...
function drawMenu() {
  ...
  ctx.font = '11px monospace'
  ctx.fillText(
    '← → / A D : Di chuyển  |  ↑ / W / Space : Nhảy  |  Z / J : Tấn công',
    W / 2,
    H / 2 + 55,
  )
...
function drawGameOver() {
  ...
  ctx.fillText(`Score: ${score.value}`, W / 2, H / 2 - 30)
  ctx.fillStyle = '#94a3b8'
  ctx.font = '14px monospace'
  ctx.fillText(
    `Level: ${level.value} | Kills: ${killCount.value} | Map: ${MAP_CONFIGS[currentMapId.value].name}`,
    W / 2,
    H / 2,
  )
...
function setupAdminMode() {
  ...
  level.value = 50
  playerMaxHp.value = 9999
  playerHp.value = 9999
  playerMaxMp.value = 9999
  playerMp.value = 9999
  player.baseAtk = 999
  ...
  const legendaryEquips: {
    id: string
    slot: EquipSlot
    name: string
    icon: string
    passive: PassiveType
    stats: EquipmentConfig['stats']
  }[] = [
    {
      id: 'shadow_hood',
      slot: 'head',
      name: 'Mũ Trùm Bóng Tối +9',
      icon: '🌑',
      passive: 'crit_burst',
      stats: { hp: 200, atk: 30, critChance: 40 },
    },
    {
      id: 'divine_robe',
      slot: 'chest',
      name: 'Áo Thần Thánh +9',
      icon: '✨',
      passive: 'mana_shield',
      stats: { def: 80, hp: 150, manaRegen: 0.15 },
    },
    {
      id: 'wind_hakama',
      slot: 'legs',
      ...
      icon: '🌪️',
      passive: 'speed_aura',
      stats: { speed: 4, def: 30, jumpPower: -5 },
    },
    {
      id: 'god_fists',
      slot: 'gloves',
      ...
      icon: '💥',
      passive: 'lightning',
      stats: { atk: 90, critChance: 40, def: 12 },
    },
    {
      id: 'celestial_step',
      slot: 'boots',
      name: 'Bước Chân Tiên +9',
      icon: '🌟',
      passive: 'lifesteal',
      stats: { speed: 3, jumpPower: -6, manaRegen: 0.1 },
    },
    {
      id: 'dragon_heart',
      slot: 'accessory',
      ...
      icon: '❤️‍🔥',
      passive: 'lifesteal',
      stats: { hp: 200, atk: 50, def: 25, critChance: 25 },
    },
  ]
  ...
  for (const eq of legendaryEquips) {
    player.equipment[eq.slot] = {
      ...
      description: '👑 Admin Equipment',
      color: '#fb923c',
    }
  }

  // Full cánh - celestial
  const allWings: WingType[] = [
    'wood_wings',
    'angel_wings',
    'dragon_wings',
    'shadow_wings',
...
function actualStartGame() {
  ...
  maxMonsters = INITIAL_MAX_MONSTERS
  activeNPC = null
  ...
  spawnFloatingText(
    player.x + player.w / 2,
    player.y - 30,
    `🏘️ Chào mừng ${playerName.value} đến Làng Ninja!`,
    '#fbbf24',
    16,
  )
}
...
function checkNPCInteraction() {
  ...
  for (const npc of npcs) {
    if (
      Math.abs(player.x + player.w / 2 - npc.x - npc.w / 2) < 50 &&
  ...
  if (gameState.value === 'menu') {
    ...
    if (!showInventory.value && !isInteracting) {
      ...
      if (currentMapId.value !== 'hub') {
        ...
        if (
          Math.abs(player.x - mapCfg2.portalX) < 40 &&
          (keyJustPressed['ArrowUp'] || keyJustPressed['KeyW'])
        ) {
          switchMap('hub')
        }
      }
    ...
    if (
      activeNPC &&
      (keyJustPressed['ArrowDown'] ||
        keyJustPressed['KeyS'] ||
        keyJustPressed['KeyB'] ||
        keyJustPressed['Escape'])
    ) {
      activeNPC.interacting = false
      activeNPC = null
      upgradeSelectedEquip = null
      upgradeResult = 'none'
    }
    ...
    if (showMapSelection) {
      ...
      if (keyJustPressed['ArrowDown'])
        mapSelectIdx = Math.min(BATTLE_MAPS.length - 1, mapSelectIdx + 1)
    ...
    if (activeNPC?.type === 'merchant') {
      ...
      drawUpgradeUI(
        ...
        W,
        H,
        upgradeSelectedEquip,
        materialInventory,
        upgradeUseRefineDust,
        upgradeUseProtection,
        getAllUpgradeableItems(),
        upgradeResult,
        upgradeAnim,
        upgradeListIdx,
      )
...
function drawTabbedInventory() {
  ...
  if (invTab === 0) {
    ...
    allWeapons.forEach((wt, i) => {
      ...
      const isHover =
        mousePos.x >= sx &&
        mousePos.x <= sx + slotSize &&
        mousePos.y >= sy &&
        mousePos.y <= sy + slotSize
    ...
    ctx.fillStyle = '#ef4444'
    ctx.fillText(`⚔ DMG: ${selCfg.damage}`, detX, detY + 52)
    ctx.fillStyle = '#3b82f6'
    ctx.fillText(`↔ Range: ${selCfg.range}`, detX, detY + 68)
    ctx.fillStyle = '#4ade80'
    ctx.fillText(`⚡ Speed: ${selCfg.speed > 0 ? '+' : ''}${selCfg.speed}`, detX, detY + 84)
    // Damage bar
    ctx.fillStyle = '#1a1a2e'
    ctx.fillRect(detX, detY + 94, 150, 6)
    ctx.fillStyle = '#ef4444'
    ctx.fillRect(detX, detY + 94, Math.min(150, selCfg.damage * 4.3), 6)
    ctx.fillStyle = '#1a1a2e'
    ctx.fillRect(detX, detY + 106, 150, 6)
    ctx.fillStyle = '#3b82f6'
    ctx.fillRect(detX, detY + 106, Math.min(150, selCfg.range * 1.87), 6)
  } else if (invTab === 1) {
    ...
    slots.forEach((slot, i) => {
      ...
      const isHov =
        mousePos.x >= sx &&
        mousePos.x <= sx + eqSlotW &&
        mousePos.y >= sy &&
        mousePos.y <= sy + eqSlotH
      ...
      if (eq) {
        ...
        ctx!.fillStyle = RARITY_COLORS[eq.rarity]
        ctx!.font = 'bold 9px monospace'
        ...
        ctx!.fillStyle = btnHov ? '#fff' : '#ef4444'
    ...
    const invX = px + 14,
      invY = eqStartY + 2 * (eqSlotH + eqGap + 18) + 16
    ctx.fillStyle = '#fbbf24'
    ctx.font = 'bold 10px monospace'
    ...
    equipInventory.slice(0, maxShow).forEach((eq, i) => {
      ...
      if (canUp) {
        ...
        ctx!.strokeStyle = '#fbbf24'
        ctx!.lineWidth = 1
        ctx!.strokeRect(btnX, ey - 2, 20, 14)
        ctx!.fillStyle = upgBtnHov ? '#fff' : '#fbbf24'
        ctx!.font = 'bold 8px monospace'
        ctx!.textAlign = 'center'
      ...
      const eqBtnHov =
        mousePos.x >= eqBtnX &&
        mousePos.x <= eqBtnX + 30 &&
        mousePos.y >= eqBtnY &&
        mousePos.y <= eqBtnY + 14
      ctx!.fillStyle = eqBtnHov ? 'rgba(74,222,128,0.4)' : 'rgba(74,222,128,0.15)'
      ctx!.fillRect(eqBtnX, eqBtnY, 30, 14)
      ctx!.strokeStyle = '#4ade80'
      ctx!.lineWidth = 1
      ctx!.strokeRect(eqBtnX, eqBtnY, 30, 14)
      ctx!.fillStyle = eqBtnHov ? '#fff' : '#4ade80'
      ctx!.font = 'bold 7px monospace'
      ctx!.textAlign = 'center'
      ...
      ctx!.strokeStyle = '#ef4444'
      ctx!.lineWidth = 1
      ctx!.strokeRect(delBtnX, eqBtnY, 24, 14)
      ctx!.fillStyle = delBtnHov ? '#fff' : '#ef4444'
      ctx!.font = 'bold 7px monospace'
    ...
    if (equipInventory.length > maxShow) {
      ...
      ctx.font = '8px monospace'
      ctx.fillText(
        `... và ${equipInventory.length - maxShow} item khác`,
        invX + 4,
        invY + 14 + maxShow * 22 + 8,
      )
    }
  } else if (invTab === 2) {
    // Consumables tab
    ...
    if (ownedWings.length === 0) {
      ...
      ownedWings.forEach((wt, i) => {
        ...
        ctx!.fillStyle = RARITY_COLORS[wCfg.rarity]
        ctx!.font = 'bold 8px monospace'
    ...
    if (equippedWing) {
      ...
      ctx.fillRect(px + 20, py + panelH - 60, panelW - 40, 40)
      ctx.strokeStyle = '#fbbf24'
      ctx.lineWidth = 1
      ctx.strokeRect(px + 20, py + panelH - 60, panelW - 40, 40)
      ctx.fillStyle = '#fbbf24'
      ctx.font = 'bold 10px monospace'
      ...
      ctx.font = '9px monospace'
      ctx.fillText(
        `+${equippedWing.extraJumps} nhảy | Giảm ${Math.round(equippedWing.floatReduction * 100)}% trọng lực | Bay ${(equippedWing.flyDuration / 60).toFixed(1)}s`,
        px + 30,
        py + panelH - 28,
      )
    }
...
function resizeCanvas() {
  ...
  W = maxW
  H = Math.floor(maxW / (CANVAS_WIDTH / CANVAS_HEIGHT))
...
<template>
  <div
    class="min-h-screen bg-[#0f0f23] text-white font-mono flex flex-col items-center justify-center px-4 py-6 relative"
  >
    <RouterLink
      to="/"
    ...
    <div class="relative w-full max-w-[960px]">
      <canvas
        ...
        :class="{
          'border-red-500/50': gameState === 'gameover',
          'border-green-500/30': gameState === 'playing',
        }"
    ...
    <div class="mt-4 flex gap-3 sm:hidden">
      <button
        class="w-14 h-14 bg-gray-800 border border-gray-600 rounded-lg text-2xl active:bg-gray-700 active:scale-95 transition"
        @touchstart.prevent="keys['ArrowLeft'] = true"
        @touchend.prevent="keys['ArrowLeft'] = false"
      >
        ←
      </button>
      <button
        class="w-14 h-14 bg-gray-800 border border-gray-600 rounded-lg text-2xl active:bg-gray-700 active:scale-95 transition"
        @touchstart.prevent="keys['ArrowRight'] = true"
        @touchend.prevent="keys['ArrowRight'] = false"
      >
        →
      </button>
      <button
        class="w-14 h-14 bg-blue-800 border border-blue-600 rounded-lg text-lg active:bg-blue-700 active:scale-95 transition"
      ...
      <button
        class="w-14 h-14 bg-red-800 border border-red-600 rounded-lg text-lg active:bg-red-700 active:scale-95 transition"
        @touchstart.prevent="keys['KeyZ'] = true"
        @touchend.prevent="keys['KeyZ'] = false"
      >
        ⚔
      </button>
    </div>
