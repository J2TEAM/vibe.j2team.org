...
<template>
  <div class="w-full flex flex-col gap-2 animate-fade-up">
    <div
      class="bg-bg-surface p-4 border border-border-default shadow-xl relative overflow-hidden group hover:border-accent-coral transition-all duration-300 w-full"
    >
      <!-- Background Number Decoration -->
      <span
        class="absolute -bottom-4 -right-2 font-display text-8xl font-bold text-accent-coral/5 select-none pointer-events-none"
      >
        02
      </span>
      ...
      <div class="flex flex-col lg:flex-row gap-4 sm:gap-6 relative z-10">
        ...
        <div class="grow flex flex-col gap-2">
          <div
            class="flex flex-col sm:flex-row items-stretch sm:items-center bg-bg-deep border border-border-default p-1 transition-colors focus-within:border-accent-coral"
          >
            <div class="flex items-center grow">
              <div
                class="flex items-center justify-center px-4 py-2 bg-accent-coral/10 text-accent-coral font-black border-r border-border-default h-full"
              >
                $
              </div>
              <input
            ...
            <div
              class="flex gap-1 p-1 bg-bg-surface/50 sm:bg-transparent border-t sm:border-t-0 border-border-default mt-1 sm:mt-0 sm:ml-auto"
            >
              <button
                @click="multiplyBet(0.5)"
                :disabled="disabled"
                class="flex-1 sm:flex-none bg-bg-elevated hover:bg-accent-coral hover:text-bg-deep text-text-secondary px-4 py-3 sm:py-2 text-[10px] font-black transition-all uppercase tracking-tighter border border-border-default active:scale-95"
              >
                ½
              </button>
              <button
