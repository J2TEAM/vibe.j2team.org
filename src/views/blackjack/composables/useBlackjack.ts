...
export function useBlackjack() {
  ...
  const split = () => {
    ...
    const v1 =
      card1.rank === 'A' ? 11 : ['J', 'Q', 'K'].includes(card1.rank) ? 10 : parseInt(card1.rank)
    const v2 =
      card2.rank === 'A' ? 11 : ['J', 'Q', 'K'].includes(card2.rank) ? 10 : parseInt(card2.rank)
    if (v1 !== v2) return
