...
const database = [
  ...
  {
    name: 'Thịt Dê Núi Chính Thư',
    dish: 'Thịt dê',
    type: 'Nhậu nhẹt',
    addr: 'Thôn Khê Thượng, Ninh Xuân',
    dist: 'Ninh Bình',
    time: '09:00 - 22:00',
    price: '150k - 300k',
  },
  {
    name: 'Miến Lươn Đồng Phú',
    dish: 'Miến lươn',
    type: 'Món nước',
    addr: '65 Tôn Thất Tùng',
    dist: 'Ninh Bình',
    time: '06:00 - 20:00',
    price: '35k - 50k',
  },
  {
    name: 'Súp Lươn Bà Đinh',
    dish: 'Súp lươn',
    type: 'Món nước',
    addr: '154 Đinh Công Tráng',
    dist: 'Nghệ An',
    time: '06:00 - 10:00',
    price: '30k - 50k',
  },
  {
    name: 'Nem Thanh Hóa Cây Đa',
    dish: 'Nem chua',
    type: 'Đặc sản',
    addr: '326 Trường Thi',
    dist: 'Thanh Hóa',
  ...
  },
  {
    ...
    dish: 'Hủ tiếu Sa Đéc',
    type: 'Món nước',
    addr: '188 Trần Hưng Đạo',
    dist: 'Đồng Tháp',
    time: '06:00 - 22:00',
    price: '20k - 30k',
  },
  {
    name: 'Bún Cá Cường Châu Đốc',
    dish: 'Bún cá',
    type: 'Món nước',
    addr: '61 Trưng Nữ Vương',
    dist: 'An Giang',
    time: '06:00 - 12:00',
    price: '25k - 40k',
  },
...
const cityMap: Record<string, string> = {
  ...
  'Tp. Vũng Tàu': 'TP. Hồ Chí Minh',
  'Thủ Dầu Một': 'TP. Hồ Chí Minh',
}

const getCity = (dist: string) => {
  if (cityMap[dist]) return cityMap[dist]
  if (
    dist.startsWith('Quận') ||
    dist.startsWith('Huyện') ||
    dist === 'TP. Thủ Đức' ||
    dist === 'Thủ Đức' ||
    dist === 'Toàn Thành Phố'
  ) {
...
const groupedLocations = computed(() => {
  ...
  allDistricts.forEach((dist) => {
    const city = getCity(dist)
    if (!groups[city]) groups[city] = []
    groups[city].push(dist)
  })
...
<template>
  <div
    class="min-h-screen bg-[#0F1923] text-[#F0EDE6] font-body flex flex-col items-center justify-between p-4 relative overflow-hidden"
  >
    <div
      class="fixed inset-0 pointer-events-none opacity-[0.025] mix-blend-overlay z-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')]"
    ></div>

    <RouterLink
      to="/"
      class="absolute top-4 left-4 border border-[#253549] bg-[#162232] text-[#8B9DB5] px-4 py-2 text-xs font-display tracking-widest transition-all duration-300 hover:-translate-y-1 hover:border-[#FF6B4A] hover:bg-[#1E2F42] hover:text-[#F0EDE6] hover:shadow-lg hover:shadow-[#FF6B4A]/10 z-10 uppercase"
    >
      &larr; TRANG CHỦ
    </RouterLink>

    <div class="w-full flex flex-col items-center mt-12 mb-6 z-10 animate-fade-up">
      <div class="relative">
        <h1
          class="font-display text-4xl md:text-5xl font-bold text-[#FF6B4A] mb-2 relative z-10 text-center"
        >
          Bé Ơi Ăn Gì?
        </h1>
        <div
          class="absolute -top-3 -right-6 bg-[#FF6B4A] text-[#0F1923] font-display font-bold text-[10px] tracking-widest px-2 py-1 rotate-3 shadow-md z-20"
        >
          V.01 / 2026
        </div>
      </div>

      <div class="flex gap-1.5 my-4 opacity-50">
        <span v-for="n in 20" :key="n" class="w-1 h-1 rounded-full bg-[#253549]"></span>
      </div>

      <button
        @click="showModal = true"
        class="mt-2 border border-[#253549] bg-[#162232] px-6 py-2.5 font-display text-sm tracking-wide text-[#38BDF8] transition-all duration-300 hover:border-[#38BDF8] hover:bg-[#1E2F42] hover:-translate-y-0.5 flex items-center gap-2"
      >
        <span class="text-[#FFB830]">//</span> CHỌN KHU VỰC
        <span class="text-[#8B9DB5] opacity-70">[{{ filterCountText }}]</span>
      </button>
    </div>

    <div
      v-if="showModal"
      class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0F1923]/90 backdrop-blur-sm"
    >
      <div
        class="w-full max-w-lg max-h-[80vh] border border-[#253549] bg-[#162232] shadow-2xl flex flex-col animate-fade-up"
      >
        <div class="p-5 border-b border-[#253549] flex justify-between items-center bg-[#1E2F42]">
          <h2 class="font-display text-lg font-semibold text-[#FFB830] flex items-center gap-2">
            <span class="text-[#FF6B4A]">//</span> TÙY CHỈNH ĐỊA ĐIỂM
          </h2>
          <button
            @click="showModal = false"
            class="text-[#8B9DB5] hover:text-[#FF6B4A] text-xl leading-none"
          >
            &times;
          </button>
        </div>
        ...
        <div
          class="overflow-y-auto p-5 flex flex-col gap-6 scrollbar-thin scrollbar-thumb-[#253549] scrollbar-track-transparent"
        >
          <div
            v-for="(districts, city) in groupedLocations"
            :key="city"
            class="flex flex-col gap-3"
          >
            <div
              class="font-display text-xs font-bold tracking-widest text-[#8B9DB5] border-b border-[#253549] pb-2 uppercase"
            >
              {{ city }}
            </div>
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
              <label
                ...
                :key="dist"
                class="relative border border-[#253549] bg-[#0F1923] p-2 text-xs text-center cursor-pointer transition-all hover:border-[#38BDF8] hover:bg-[#1E2F42]"
                :class="{
                  '!border-[#FF6B4A] !bg-[#FF6B4A]/10 text-[#FF6B4A] font-medium':
                    selectedDistricts.includes(dist),
                }"
              >
                <input type="checkbox" :value="dist" v-model="selectedDistricts" class="hidden" />
                <span class="select-none truncate block w-full" :title="dist">{{ dist }}</span>
              </label>
            </div>
          </div>
        </div>

        <div class="p-5 border-t border-[#253549] flex gap-3 bg-[#162232]">
          <button
            @click="toggleAll"
            class="flex-1 border border-[#253549] bg-[#0F1923] py-2.5 text-sm font-display tracking-wide text-[#8B9DB5] hover:text-[#F0EDE6] hover:bg-[#1E2F42] transition-colors"
          >
            {{ selectedDistricts.length === allDistricts.length ? 'BỎ CHỌN' : 'CHỌN TẤT CẢ' }}
          </button>
          <button
            @click="showModal = false"
            class="flex-1 border border-[#FF6B4A] bg-[#FF6B4A]/10 py-2.5 text-sm font-display font-semibold tracking-wide text-[#FF6B4A] hover:bg-[#FF6B4A] hover:text-[#0F1923] transition-colors"
          >
            XÁC NHẬN
          </button>
        </div>
    ...
    <div
      class="w-full max-w-sm flex-grow flex items-center justify-center relative perspective-1000 z-10 animate-fade-up animate-delay-3"
    >
      <div
        class="w-full relative preserve-3d transition-transform duration-700 ease-[cubic-bezier(0.34,1.56,0.64,1)] aspect-[3/4] max-h-[500px]"
        :class="{ '[transform:rotateY(180deg)]': isFlipped }"
      >
        <div
          class="absolute inset-0 backface-hidden border border-[#253549] bg-[#162232] flex items-center justify-center group overflow-hidden hover:border-[#FF6B4A] transition-colors duration-300"
        >
          <div
          ...
          ></div>
          <div
            class="font-display font-bold text-2xl text-[#4A6180] tracking-[0.2em] -rotate-45 group-hover:text-[#FF6B4A]/50 transition-colors"
          >
            MTDES23.ID.VN
          </div>
          <div
            class="absolute bottom-4 right-4 text-[#FFB830] font-display text-sm tracking-widest"
          >
            // J2TEAM
          </div>
        </div>
        ...
        <div
          class="absolute inset-0 backface-hidden [transform:rotateY(180deg)] border border-[#FF6B4A] bg-[#162232] flex flex-col p-6 overflow-hidden shadow-2xl shadow-[#FF6B4A]/10"
        >
          <span
            class="absolute -top-4 -right-2 font-display text-8xl font-bold text-[#FFB830]/5 select-none pointer-events-none"
            >01</span
          >

          <div class="flex-grow flex flex-col">
            <div class="flex justify-between items-start mb-4">
              <div class="text-4xl filter drop-shadow-md">{{ mascotIcon }}</div>
              <div
                class="border border-[#253549] bg-[#0F1923] text-[#8B9DB5] text-[10px] font-display tracking-widest px-2 py-1 uppercase"
              >
                {{ result.type }}
              </div>
            </div>
            ...
            <div
              class="font-display text-sm font-semibold text-[#FFB830] mb-1 uppercase tracking-wide flex items-center gap-2"
            >
              <span class="text-[#FF6B4A] text-xs">//</span> {{ result.dish }}
            </div>

            <h3 class="font-display text-2xl font-bold text-[#F0EDE6] leading-tight mb-2">
              {{ result.name }}
            </h3>

            <p class="text-[#8B9DB5] text-sm mb-6 pb-4 border-b border-[#253549] border-dashed">
              {{ result.addr }}
            </p>

            <div class="grid gap-4 mt-auto">
              <div class="flex items-center gap-3">
                <div class="w-1.5 h-1.5 bg-[#38BDF8]"></div>
                <div class="text-[#4A6180] text-xs font-display tracking-widest w-16 uppercase">
                  Khu vực
                </div>
                <div class="text-[#F0EDE6] text-sm font-medium">{{ result.dist }}</div>
              </div>
              <div class="flex items-center gap-3">
                <div class="w-1.5 h-1.5 bg-[#FFB830]"></div>
                <div class="text-[#4A6180] text-xs font-display tracking-widest w-16 uppercase">
                  Giờ mở
                </div>
                <div class="text-[#F0EDE6] text-sm font-medium">{{ result.time }}</div>
              </div>
              <div class="flex items-center gap-3">
                <div class="w-1.5 h-1.5 bg-[#FF6B4A]"></div>
                <div class="text-[#4A6180] text-xs font-display tracking-widest w-16 uppercase">
                  Mức giá
                </div>
                <div class="text-[#FF6B4A] text-sm font-bold">{{ result.price }}</div>
              </div>
            </div>
          </div>
          ...
          <a
            ...
            class="mt-6 border border-[#FF6B4A] bg-[#FF6B4A]/10 text-[#FF6B4A] py-3 text-center font-display font-semibold tracking-wide text-sm transition-all hover:bg-[#FF6B4A] hover:text-[#0F1923] uppercase"
          >
            BẢN ĐỒ
          </a>
        </div>
    ...
    <div class="w-full flex justify-center mt-6 mb-8 z-10 animate-fade-up animate-delay-4">
      <button
        @click="pickFood"
        class="border border-[#FFB830] bg-[#FFB830]/10 px-8 py-4 font-display font-bold tracking-widest text-[#FFB830] transition-all duration-300 hover:-translate-y-1 hover:bg-[#FFB830] hover:text-[#0F1923] hover:shadow-lg hover:shadow-[#FFB830]/20 flex items-center gap-3 uppercase"
      >
        <span v-if="!isFlipped">TÌM MÓN ĂN</span>
        <span v-else>ĐỔI MÓN KHÁC</span>
        <svg
...
<style scoped>
/* Reset base styles imported from main file if any */
h1,
h2,
h3,
p,
...
/* Custom classes not covered entirely by Tailwind standard tokens */
.font-display {
