  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.record && Array.isArray(data.record)) {
        ...
        const isNameTaken = data.record.some((p: {name: string, score: number}) => p.name.toLowerCase() === newName.toLowerCase())
        if (isNameTaken) {
...
const triggerLightningEffect = () => {
  ...
  setTimeout(() => { isLightning.value = false }, 500)
}
  ...
  for (let i = 0; i < 30; i++) {
    ...
    setTimeout(() => {
      flyingKarmas.value.push({ 
        ...
        y: randomY, 
        text: '+100 CÔNG ĐỨC 🌟', 
        colorClass: 'text-yellow-400 drop-shadow-[0_0_10px_rgba(251,191,36,0.8)] text-2xl sm:text-3xl font-black z-[100] animate-float-up-fast',
        delay: 0
      })
      setTimeout(() => {
        flyingKarmas.value = flyingKarmas.value.filter(k => k.id !== id)
...
const offerings = [
  { id: 'ram', name: 'RAM Cháy', icon: '💾', desc: 'RAM 16GB cháy khét lẹt. Cúng để cầu đừng tràn bộ nhớ rò rỉ.', style: { top: '10px', left: '10%', zIndex: 10, transform: 'rotate(-15deg)' } },
  { id: 'keyboard', name: 'Phím Liệt', icon: '⌨️', desc: 'Bàn phím cơ liệt nút Space. Cúng để tránh gõ nhầm lệnh rm -rf.', style: { top: '-15px', left: '30%', zIndex: 12, transform: 'rotate(5deg)' } },
  ...
  { id: 'chicken', name: 'Gà Luộc', icon: '🐔', desc: 'Gà trống ngậm hoa hồng. Cúng mặn cho dự án không bị ngỏm.', style: { top: '-5px', left: '50%', zIndex: 15, transform: 'rotate(-5deg) scale(1.2)' } },
  { id: 'router', name: 'Router Đứt', icon: '📡', desc: 'Router mạng gãy ăng ten. Cúng xin mạng thông suốt lúc push code.', style: { top: '25px', left: '40%', zIndex: 14, transform: 'rotate(-5deg)' } },
  ...
  { id: 'fruit', name: 'Mâm Ngũ Quả', icon: '🍎', desc: 'Mâm quả tươi xanh. Cúng xin pipeline CI/CD luôn một màu xanh.', style: { top: '20px', left: '60%', zIndex: 13, transform: 'rotate(10deg) scale(1.1)' } },
  { id: 'hdd', name: 'Ổ Cứng Bad', icon: '💿', desc: 'Ổ cứng kêu lạch cạch. Cúng cầu cho Database không bị corrupt.', style: { top: '20px', left: '0%', zIndex: 16, transform: 'rotate(30deg)' } },
  { id: 'usb', name: 'USB Boot', icon: '💽', desc: 'USB cài Win cứu hộ. Vật phẩm phòng thân khi màn hình xanh hiện lên.', style: { top: '25px', left: '80%', zIndex: 12, transform: 'rotate(-25deg)' } },
]
...
<template>
  <div class="min-h-screen bg-bg-deep text-text-primary font-body flex flex-col items-center justify-center px-4 py-10 relative overflow-hidden selection:bg-accent-coral/30" :class="{ 'animate-shake-violent': isJackpotShaking }">
    ...
    <!-- Hiệu ứng Sét Đánh -->
    <div v-if="isLightning" class="fixed inset-0 z-[300] bg-white mix-blend-difference animate-lightning pointer-events-none"></div>
    ...
    <div class="absolute top-4 right-4 bg-bg-surface/80 backdrop-blur border border-accent-amber/50 px-4 py-2 rounded-xl shadow-[0_0_10px_rgba(251,191,36,0.2)] z-20 flex flex-col items-end transition-transform" :class="{ 'scale-110 shadow-[0_0_30px_rgba(251,191,36,0.8)] border-yellow-400': isJackpotShaking }">
      ...
      <div class="text-[10px] text-text-dim mt-1 uppercase tracking-wider font-bold">{{ karmaTitle }}</div>
    </div>
    ...
    <div v-if="showLeaderboard" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <!-- Modal Content -->
      <div class="bg-gradient-to-b from-gray-900 to-black border-2 border-yellow-600 rounded-xl p-6 w-full max-w-md relative z-10 shadow-[0_0_50px_rgba(251,191,36,0.2)] overflow-hidden">
        <div class="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-transparent via-yellow-500 to-transparent"></div>
        <div v-if="isSyncing" class="absolute top-0 left-0 h-1 bg-yellow-400 animate-pulse w-full"></div>
        
        <button @click="showLeaderboard = false" class="absolute top-3 right-4 text-gray-500 hover:text-white text-xl font-bold">&times;</button>
        
        <h2 class="text-3xl font-display font-bold text-center text-yellow-500 mb-6 drop-shadow-[0_2px_5px_rgba(251,191,36,0.5)] tracking-widest uppercase">Bảng Phong Thần</h2>
        
        <!-- Nhập tên -->
        <div v-if="isEditingName" class="mb-6 p-4 bg-gray-800/50 rounded-lg border border-gray-700">
          <p class="text-sm text-gray-300 mb-2 italic">Xin cho biết quý danh để lưu vào sổ Công Đức:</p>
          <div class="flex gap-2">
            <input 
              ...
              maxlength="20"
            >
            <button @click="saveName" class="bg-yellow-600 hover:bg-yellow-500 text-black font-bold px-4 py-2 rounded transition-colors" :disabled="isSyncing">Lưu</button>
          </div>
        </div>
        <div v-else class="mb-6 text-center text-gray-300 text-sm flex justify-center items-center gap-2">
          Danh xưng: <span class="font-bold text-yellow-400 text-base">{{ playerName }}</span>
          <button @click="isEditingName = true" class="text-gray-500 hover:text-yellow-400" title="Đổi tên">✏️</button>
          <button @click="fetchLeaderboard" class="ml-2 text-gray-500 hover:text-green-400" title="Làm mới bảng" :class="{'animate-spin': isSyncing}">🔄</button>
        </div>

        <!-- Danh sách Top -->
        <div class="space-y-2 max-h-[40vh] overflow-y-auto pr-2 custom-scrollbar">
          <div v-if="leaderboardData.length === 0 && !isSyncing" class="text-center text-gray-500 py-4 italic">Chưa có vị thần nào giáng thế...</div>
          <div v-if="isSyncing && leaderboardData.length === 0" class="text-center text-yellow-500 py-4 italic animate-pulse">Đang thỉnh sớ từ cõi Server...</div>
          ...
          >
            <div class="flex items-center gap-3">
              <!-- Hạng -->
              <span class="w-6 text-center font-black text-lg" :class="index === 0 ? 'text-yellow-400' : (index === 1 ? 'text-gray-300' : (index === 2 ? 'text-amber-600' : 'text-gray-600'))">
                {{ index === 0 ? '🥇' : (index === 1 ? '🥈' : (index === 2 ? '🥉' : '#' + (index + 1))) }}
              </span>
              <span class="font-bold truncate max-w-[150px] sm:max-w-[200px]" :class="{ 'font-display tracking-wide': player.name === playerName }">
                {{ player.name }} <span v-if="player.name === playerName" class="text-xs text-yellow-500/70 ml-1">(Bạn)</span>
              </span>
            </div>
        ...
        <p class="text-center text-xs text-gray-600 mt-4 italic">*Hack điểm sẽ bị thần linh quật cháy RAM*</p>
      </div>
    </div>

    <!-- Màn hình đỏ ngòm khi dính Easter Egg -->
    <div v-if="isBugEasterEgg" class="fixed inset-0 z-[200] bg-red-900/90 flex flex-col items-center justify-center animate-shake-violent pointer-events-none">
      <div class="text-9xl animate-bounce drop-shadow-[0_0_50px_rgba(255,0,0,1)]">🐞</div>
      <h2 class="text-white text-5xl font-black mt-8 font-display drop-shadow-lg text-center uppercase tracking-widest">Nghiệp Quật!</h2>
      <p class="text-yellow-300 text-2xl mt-4 font-bold bg-black/50 px-6 py-2 rounded-lg border border-red-500">-50 Công Đức</p>
    ...
    <div class="relative w-full max-w-2xl flex flex-col items-center mt-10 z-10" :class="{ 'shake-little border-red-500/50': isPraying }">
      ...
      <div v-if="incenses > 0" class="fixed inset-0 z-40 pointer-events-none overflow-hidden mix-blend-screen transition-all duration-1000" :style="{ opacity: 0.3 + (incenses * 0.15) }">
        
        <!-- Khói lớp 1 (Nén hương 1) - Xoáy nhẹ ở nền -->
        <div class="absolute bottom-[10%] left-[-20%] w-[150%] h-[60vh] bg-gradient-to-r from-transparent via-white/10 to-transparent blur-[50px] animate-smoke-wave-1 transform -rotate-6"></div>
        <div class="absolute top-[20%] left-[10%] w-[80vw] h-[40vh] bg-white/5 blur-[60px] rounded-full animate-smoke-swirl" style="animation-duration: 40s;"></div>
        
        <!-- Khói lớp 2 (Nén hương 2) - Cơn sóng ngược -->
        <div v-if="incenses >= 2">
          <div class="absolute bottom-[-10%] right-[-50%] w-[200%] h-[50vh] bg-gradient-to-l from-transparent via-white/15 to-transparent blur-[60px] animate-smoke-wave-2 transform rotate-12"></div>
          <div class="absolute bottom-[40%] right-[20%] w-[50vw] h-[50vh] bg-white/10 blur-[70px] rounded-full animate-smoke-swirl" style="animation-duration: 35s; animation-direction: reverse;"></div>
        </div>
        
        <!-- Khói lớp 3 (Nén hương 3) - Không gian lãng đãng -->
        <div v-if="incenses >= 3">
          <div class="absolute bottom-[30%] left-[5%] w-[90vw] h-[40vh] bg-white/15 blur-[80px] rounded-[100%] animate-smoke-swirl" style="animation-duration: 25s;"></div>
          <!-- Lớp sương mù che phủ nhẹ toàn bộ -->
          <div class="absolute bottom-0 left-[-50%] w-[200%] h-[70vh] bg-gradient-to-t from-white/10 to-transparent blur-[40px] animate-smoke-wave-1" style="animation-duration: 45s; transform: rotate(0deg);"></div>
        </div>
      </div>
      ...
      <div class="absolute inset-0 pointer-events-none -m-4 sm:-m-8 z-20">
        ...
        <div class="absolute top-16 sm:top-24 left-0 w-full flex justify-between px-4 opacity-90 z-20">
          <div class="w-16 sm:w-28 h-40 sm:h-56 bg-gradient-to-b from-red-800 to-red-900 rounded-b-full shadow-2xl border-b-[3px] border-yellow-500 origin-top animate-curtain-sway flex flex-col items-center pt-4" style="animation-delay: 0s;">
             <div class="w-1 h-[80%] bg-yellow-600/30 rounded-full"></div>
          </div>
          <div class="w-24 sm:w-40 h-24 sm:h-32 bg-gradient-to-b from-red-800 to-red-900 rounded-b-full shadow-2xl border-b-[3px] border-yellow-500 origin-top animate-curtain-sway" style="animation-delay: 0.5s;"></div>
          <div class="w-16 sm:w-28 h-40 sm:h-56 bg-gradient-to-b from-red-800 to-red-900 rounded-b-full shadow-2xl border-b-[3px] border-yellow-500 origin-top animate-curtain-sway flex flex-col items-center pt-4" style="animation-delay: 1s;">
             <div class="w-1 h-[80%] bg-yellow-600/30 rounded-full"></div>
          </div>
        </div>

        <!-- Hai cột trụ chạm rồng -->
        <div class="absolute top-16 sm:top-24 bottom-0 left-0 w-8 sm:w-12 bg-gradient-to-r from-red-950 via-red-800 to-red-900 border-x-2 border-yellow-700 rounded-t shadow-2xl flex flex-col items-center py-4 z-10">
          <div class="text-yellow-500/60 text-xl sm:text-2xl write-vertical font-serif tracking-widest h-full flex items-center justify-center font-bold drop-shadow-md">CODE MƯỢT MÀ</div>
        </div>
        <div class="absolute top-16 sm:top-24 bottom-0 right-0 w-8 sm:w-12 bg-gradient-to-l from-red-950 via-red-800 to-red-900 border-x-2 border-yellow-700 rounded-t shadow-2xl flex flex-col items-center py-4 z-10">
          <div class="text-yellow-500/60 text-xl sm:text-2xl write-vertical font-serif tracking-widest h-full flex items-center justify-center font-bold drop-shadow-md">SERVER VỮNG VÀNG</div>
        </div>
      </div>

      <!-- Mặt bàn thờ (Main content) -->
      <div class="bg-gradient-to-b from-gray-900/90 to-black/90 backdrop-blur-md border-2 border-yellow-700/50 p-6 sm:p-10 pt-32 sm:pt-44 shadow-[0_10px_40px_rgba(251,191,36,0.15)] w-[90%] mx-auto flex flex-col items-center relative z-10 mt-8">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 w-full">
        ...
        <button 
          ...
          :disabled="isBowlExploded"
        >
          {{ isBurningPaper ? '🛑 Dừng hóa vàng (Tự động canh tác công đức)' : '🧧 Hóa Vàng Mã (Cần 500 Công Đức)' }}
        </button>
      </div>
    ...
    <div class="fixed left-4 top-1/2 -translate-y-1/2 w-64 hidden lg:block z-40">
      <div class="bg-[#f4e4bc] p-5 rounded-sm shadow-[0_10px_30px_rgba(0,0,0,0.5)] border-x-[12px] border-[#c0392b] relative font-sans text-[#5c3a21] transform -rotate-2">
        <!-- Trục cuốn sớ -->
        <div class="absolute -top-3 -left-4 -right-4 h-6 bg-gradient-to-b from-[#8b4513] to-[#5c2e0b] rounded-full shadow-md border border-[#3e1f07]"></div>
        <div class="absolute -bottom-3 -left-4 -right-4 h-6 bg-gradient-to-b from-[#8b4513] to-[#5c2e0b] rounded-full shadow-md border border-[#3e1f07]"></div>
        
        <h3 class="text-center font-bold text-xl text-[#8b0000] border-b-2 border-[#8b0000]/30 pb-2 mb-4 uppercase tracking-wider font-display">Tâm Pháp Tu Luyện</h3>
        
        <ul class="space-y-3 text-sm leading-relaxed">
          <li class="flex items-start gap-2">
            <span class="text-[#8b0000] mt-0.5">📜</span>
            <span><strong>B1:</strong> Thành tâm chọn một món <span class="font-bold text-[#b85a1b]">đồ cúng</span> phù hợp với "Nghiệp" của dự án.</span>
          </li>
          <li class="flex items-start gap-2">
            <span class="text-[#8b0000] mt-0.5">🔥</span>
            <span><strong>B2:</strong> <span class="font-bold text-[#b85a1b]">Thắp hương</span> để thông linh tới cõi Server. Mọi lời cầu xin lúc này mới linh nghiệm.</span>
          </li>
          <li class="flex items-start gap-2">
            <span class="text-[#8b0000] mt-0.5">🙏</span>
            <span><strong>B3:</strong> Bấm <span class="font-bold text-[#b85a1b]">Khấn Cáo</span> để giãi bày nỗi lòng Coder.</span>
          </li>
          <li class="flex items-start gap-2">
            <span class="text-[#8b0000] mt-0.5">☯️</span>
            <span><strong>B4:</strong> <span class="font-bold text-[#b85a1b]">Xin Keo</span> để biết Thần Linh có cho phép nhấn nút Deploy hôm nay hay không.</span>
          </li>
          <li class="flex items-start gap-2 border-t border-[#8b0000]/20 pt-3 mt-2">
            <span class="text-[#8b0000] mt-0.5">🐟</span>
            <span>Gõ mõ liên tục để thăng cấp cảnh giới từ "Thực Tập Sinh" thành "Thích Ca Mâu Ni Coder".</span>
          </li>
        </ul>

        <div class="mt-5 p-3 bg-[#e8d5a5] border border-[#d4c08f] rounded italic text-xs text-center text-[#8b0000]/80">
    ...
    <div class="fixed right-4 top-1/2 -translate-y-1/2 w-64 hidden xl:block z-40">
      <div class="bg-[#f4e4bc] p-5 rounded-sm shadow-[0_10px_30px_rgba(0,0,0,0.5)] border-x-[12px] border-[#c0392b] relative font-sans text-[#5c3a21] transform rotate-2 min-h-[460px] flex flex-col justify-between">
        <!-- Trục cuốn sớ -->
        <div class="absolute -top-3 -left-4 -right-4 h-6 bg-gradient-to-b from-[#8b4513] to-[#5c2e0b] rounded-full shadow-md border border-[#3e1f07]"></div>
        <div class="absolute -bottom-3 -left-4 -right-4 h-6 bg-gradient-to-b from-[#8b4513] to-[#5c2e0b] rounded-full shadow-md border border-[#3e1f07]"></div>
        
        <h3 class="text-center font-bold text-xl text-[#8b0000] border-b-2 border-[#8b0000]/30 pb-2 mb-4 uppercase tracking-wider font-display">Luật Nhân Quả</h3>
        
        <div class="space-y-4 text-sm leading-relaxed text-center flex-1">
          <p class="italic text-[#8b0000]/90 mb-2">
            "Có thờ có thiêng,<br>có kiêng có lành..."
          </p>
          
          <div class="bg-[#e8d5a5] p-3 border border-[#d4c08f] rounded shadow-inner relative overflow-hidden group mb-4">
            ...
            <p class="text-[11px] leading-tight">Trúng Nổ Hũ! Mưa Công Đức và Đồng Xu rơi lả tả. Lập tức nhận <strong>+100 Công Đức</strong>.</p>
          </div>

          <div class="text-left space-y-3 mt-2 text-xs border-y border-[#8b0000]/20 py-3">
            <p class="flex gap-2 items-start"><span class="text-[#8b0000] text-base">☯️</span> <span><strong>Nhất Âm Nhất Dương:</strong><br>Keo tốt. Mọi sự hanh thông, được phép Deploy lên Production.</span></p>
            <p class="flex gap-2 items-start"><span class="text-gray-600 text-base">⚫</span> <span><strong>Hai Âm (Úp):</strong><br>Thần linh quay lưng. Hệ thống báo động đỏ, cố push là sập Server.</span></p>
            <p class="flex gap-2 items-start"><span class="text-gray-600 text-base">⚪</span> <span><strong>Hai Dương (Ngửa):</strong><br>Thần linh đang cười chê. Code còn quá nhiều Bug, về đi fix lại!</span></p>
          </div>
          
        </div>
        
        <div class="mt-4 pt-3 border-t-2 border-dashed border-[#8b0000]/30 text-xs font-bold text-[#8b0000] text-center italic bg-[#e8d5a5]/50 p-2 rounded">
          "Khuyên ai chăm chỉ thắp hương,<br>Gõ mõ tích đức, dọn đường cõi IT.<br>Xin Keo thành ý khắc ghi,<br>Ắt có ngày nổ, mây đi nắng về!"
        </div>
      </div>
    </div>
...
@keyframes spread-up {
  ...
  100% { transform: translate(calc(-20px + 40px * var(--tx, 0.5)), -80px) scale(1.5); opacity: 0; }
}
.animate-spread-up {
...
@keyframes smoke-wave-1 {
  0% { transform: translateX(-30%) rotate(-10deg) scaleY(1); opacity: 0; }
  20% { opacity: 0.6; }
  50% { transform: translateX(10%) rotate(-15deg) scaleY(1.5); opacity: 0.8; }
  80% { opacity: 0.6; }
  100% { transform: translateX(50%) rotate(-10deg) scaleY(1); opacity: 0; }
}
.animate-smoke-wave-1 { animation: smoke-wave-1 20s ease-in-out infinite; }

@keyframes smoke-wave-2 {
  0% { transform: translateX(30%) rotate(5deg) scaleY(1); opacity: 0; }
  20% { opacity: 0.5; }
  50% { transform: translateX(-10%) rotate(10deg) scaleY(1.2); opacity: 0.7; }
  80% { opacity: 0.5; }
  100% { transform: translateX(-50%) rotate(5deg) scaleY(1); opacity: 0; }
}
.animate-smoke-wave-2 { animation: smoke-wave-2 25s ease-in-out infinite; }

@keyframes smoke-swirl {
  0% { transform: scale(1) rotate(0deg) translateY(0); opacity: 0.3; }
  50% { transform: scale(1.3) rotate(180deg) translateY(-50px); opacity: 0.6; }
  100% { transform: scale(1) rotate(360deg) translateY(0); opacity: 0.3; }
}
.animate-smoke-swirl { animation: smoke-swirl 30s linear infinite; }
@keyframes lightning {
  0% { opacity: 0; }
  10% { opacity: 1; }
  20% { opacity: 0; }
...
.animate-lightning { animation: lightning 0.5s ease-out forwards; }
