<template>
  <div class="min-h-screen bg-[#0b0e11] text-[#eaecef] font-sans selection:bg-yellow-500/30 overflow-hidden">
    
    <div v-if="currentView === 'Lobby'" class="fixed inset-0 z-50 bg-[#0b0e11] flex items-center justify-center p-4">
      <div class="bg-[#161a1e] p-10 rounded-3xl border border-gray-800 shadow-2xl w-full max-w-md text-center border-t-yellow-500/50">
        <h1 class="text-3xl font-black text-white mb-2 italic tracking-tighter uppercase">GIẢ LẬP<span class="text-yellow-500 ml-2 font-black">LÀM GIÀU</span></h1>
        <p class="text-gray-500 text-[10px] mb-10 uppercase tracking-[0.3em]">Blind Box Edition - Thử vận may tỷ phú</p>
        
        <div class="space-y-5 text-left mb-8">
          <div>
            <label class="text-[10px] text-gray-500 uppercase font-black ml-1 tracking-widest">Tên nhà giao dịch</label>
            <input v-model="userProfile.name" type="text" placeholder="Nhập tên của bạn..." 
        ...
        <div class="space-y-4">
          <button @click="handleStartInvestment" 
                  :class="userProfile.name.trim() ? 'bg-yellow-500 text-black' : 'bg-gray-800 text-gray-500 cursor-not-allowed'"
                  class="w-full font-black py-4 rounded-xl transition-all active:scale-95 shadow-xl uppercase tracking-widest text-sm">
            Bắt đầu đầu tư
          </button>
          
          <button @click="goToView('Mining')" :disabled="!userProfile.name.trim()"
    ...
    <div v-else-if="currentView === 'Mining'" class="min-h-screen relative bg-[#0a0a0a] flex flex-col items-center overflow-hidden">
      <div class="mt-10 text-center z-20">
        <h2 class="text-2xl font-black text-purple-500 uppercase italic">Kho Báu Huyền Bí</h2>
        <p class="text-white/40 text-[10px] tracking-widest uppercase">Thả câu vào sương mù - 100% trúng quà</p>
      </div>

      <div class="absolute top-32 w-1 bg-gray-500 origin-top z-30"
           :style="{ 
             height: hookLength + 'px', 
             transform: `rotate(${hookAngle}deg)`,
             transition: isHookMoving ? 'none' : 'transform 0.05s linear'
           }">
        <div class="absolute bottom-[-15px] left-1/2 -translate-x-1/2 text-3xl drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]">
          {{ isRetracting && currentLoot ? (currentLoot.type === 'diamond' ? '💎' : currentLoot.type === 'gold' ? '💰' : '🥉') : '🪝' }}
        </div>
      </div>

      <div class="absolute inset-0 top-1/2 flex items-center justify-center">
        <div class="absolute inset-0 bg-gradient-to-t from-black via-purple-950/20 to-transparent z-10 opacity-80 animate-pulse"></div>
      ...
      <div class="absolute bottom-10 flex flex-col items-center gap-4 z-20">
        <button @click="throwHook" :disabled="isHookMoving"
                class="bg-purple-600 px-12 py-5 rounded-full font-black text-white hover:bg-purple-500 active:scale-90 transition-all uppercase tracking-widest shadow-[0_0_30px_rgba(147,51,234,0.3)]">
          {{ isHookMoving ? 'ĐANG KÉO QUÀ...' : 'THẢ CÂU BỐC TÚI MÙ' }}
        </button>
        <button @click="goToView('Lobby')" class="text-white/40 text-xs uppercase hover:text-white">Rút lui</button>
      </div>

      <div class="absolute top-5 right-5 text-2xl font-mono font-black text-purple-500">{{ balance.toLocaleString() }}$</div>
    </div>

    <div v-else-if="currentView === 'Crushing'" class="min-h-screen flex flex-col items-center justify-center bg-[#050505]">
      <div class="text-center mb-10">
        <h2 class="text-3xl font-black text-white uppercase italic">Giải mã túi mù</h2>
        <p class="text-purple-400 font-bold uppercase tracking-widest text-xs italic">Đập 10 phát để nhận kho báu X10</p>
      </div>

      <div class="relative group cursor-pointer scale-125" @click="handleHit">
        <div class="text-[10rem] select-none transition-transform active:scale-90 duration-75 relative">
        ...
        <div class="w-full bg-gray-900 h-2 rounded-full mt-10 border border-purple-900/30">
          <div class="h-full bg-purple-500 shadow-[0_0_10px_#a855f7]" :style="{ width: (hitCount * 10) + '%' }"></div>
        </div>
      </div>

      <div v-for="note in moneyNotes" :key="note.id" 
    ...
    <div v-else class="p-4 max-w-[1600px] mx-auto animate-in fade-in duration-500">
      <header class="flex flex-wrap justify-between items-center mb-6 border-b border-gray-800 pb-4 gap-4">
        <div class="flex items-center gap-4">
          <button @click="currentView = 'Lobby'" class="bg-gray-800 w-9 h-9 flex items-center justify-center rounded-xl hover:bg-gray-700 transition-all border border-gray-700 text-lg">←</button>
          <h1 class="text-xl font-black text-white italic tracking-tighter uppercase">VIBE<span class="text-yellow-500">TERMINAL</span></h1>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div class="lg:col-span-3 bg-[#161a1e] rounded-2xl p-5 border border-gray-800 shadow-xl overflow-hidden order-2 lg:order-1">
          <h2 class="text-[10px] font-black mb-5 text-blue-500 uppercase tracking-widest">🏆 Bảng xếp hạng doanh nhân</h2>
          <div class="space-y-2 overflow-y-auto max-h-[600px] pr-1 custom-scroll">
            <div v-for="(player, idx) in leaderboard" :key="player.name" 
                 ...
                 :class="player.isUser ? 'bg-yellow-500/10 border-yellow-500/40' : 'bg-[#0b0e11]/50 border-transparent'">
              <div class="flex flex-col">
                <span class="text-sm font-bold" :class="player.isUser ? 'text-yellow-500' : 'text-gray-300'">{{ idx + 1 }}. {{ player.name }}</span>
                <span class="text-[8px] uppercase text-gray-600 font-bold">{{ player.level }}</span>
              </div>
              <span class="font-mono text-[11px] font-bold">{{ Math.floor(player.totalValue).toLocaleString() }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="lg:col-span-6 space-y-6 order-1 lg:order-2">
          <div class="bg-[#161a1e] rounded-3xl p-6 border border-gray-800 shadow-2xl relative">
            <div class="flex justify-between items-start mb-6">
              <div>
                <h2 class="text-4xl font-black text-white italic tracking-tighter">{{ selectedStock.symbol }}</h2>
                <p class="text-[10px] text-gray-500 uppercase tracking-widest mt-1">{{ selectedStock.name }}</p>
              </div>
              <div class="text-right font-mono">
                ...
                <p class="text-xs font-bold" :class="selectedStock.change >= 0 ? 'text-green-500' : 'text-red-500'">{{ selectedStock.change >= 0 ? '▲' : '▼' }} {{ Math.abs(selectedStock.change) }}%</p>
              </div>
            </div>

            <div class="h-64 w-full bg-[#0b0e11] rounded-2xl mb-8 relative p-4 border border-gray-800">
              <svg class="w-full h-full" preserveAspectRatio="none" viewBox="0 0 100 100">
                <g v-for="(candle, i) in candleData" :key="i"
                  ...
                  <line :x1="getIdxX(i)" :y1="getRawY(candle.high)" :x2="getIdxX(i)" :y2="getRawY(candle.low)" :stroke="candle.close >= candle.open ? '#22c55e' : '#ef4444'" stroke-width="0.3" />
                  <rect :x="getIdxX(i) - 1" :y="getRawY(Math.max(candle.open, candle.close))" width="2" :height="Math.max(0.5, Math.abs(getRawY(candle.open) - getRawY(candle.close)))" :fill="candle.close >= candle.open ? '#22c55e' : '#ef4444'" />
                </g>
              </svg>
              <div v-if="hoveredCandle" class="absolute z-10 bg-[#1e2329] border border-gray-700 p-2 rounded text-[9px] font-mono shadow-2xl" :style="{ left: hoveredCandle.x + '%', top: '10%' }">
                O: {{ hoveredCandle.open.toLocaleString() }} | C: {{ hoveredCandle.close.toLocaleString() }}
              </div>
            </div>
            ...
            <div class="bg-[#0b0e11] p-6 rounded-2xl border border-gray-800">
              <div class="flex flex-col sm:flex-row gap-4 items-end font-mono">
                <div class="flex-1 w-full">
                  <label class="text-[10px] text-gray-500 uppercase font-black mb-2 block tracking-widest">Khối lượng (Lot Size)</label>
                  <input v-model.number="tradeAmount" type="number" min="1" class="w-full bg-[#161a1e] border border-gray-800 rounded-xl px-4 py-4 text-xl focus:border-yellow-500 outline-none text-white font-bold" />
                </div>
                <div class="flex gap-3 w-full sm:w-auto">
                  <button @click="buyStock" :disabled="balance < (selectedStock.price * tradeAmount)" class="flex-1 sm:w-32 bg-green-600 hover:bg-green-500 disabled:opacity-20 py-4 rounded-xl font-black text-lg active:scale-95 transition-all">BUY</button>
                  <button @click="sellStock" :disabled="currentStockHolding <= 0" class="flex-1 sm:w-32 bg-red-600 hover:bg-red-700 disabled:opacity-20 py-4 rounded-xl font-black text-lg active:scale-95 transition-all">SELL</button>
                </div>
              </div>
            </div>
          </div>

          <div class="grid grid-cols-2 sm:grid-cols-5 gap-2 overflow-y-auto max-h-40 p-1 custom-scroll">
            <div v-for="s in stocks" :key="s.symbol" @click="selectedStock = s"
                 ...
                 :class="selectedStock.symbol === s.symbol ? 'bg-yellow-500 text-black border-yellow-400 font-bold' : 'bg-[#161a1e] border-gray-800 text-gray-500 hover:border-gray-600'">
        ...
        <div class="lg:col-span-3 space-y-6 order-3 lg:order-3">
          <div class="bg-[#161a1e] rounded-2xl p-6 border border-gray-800 shadow-xl overflow-hidden h-fit">
            <h2 class="text-[10px] font-black mb-6 uppercase text-gray-500 tracking-widest flex justify-between">Vị thế mở <span class="text-blue-400 font-mono">{{ portfolio.length }}</span></h2>
            <div v-if="portfolio.length === 0" class="text-center py-24 opacity-10 uppercase text-[10px] font-bold">Danh mục trống</div>
            <div v-else class="space-y-4">
              <div v-for="item in portfolio" :key="item.symbol" class="bg-[#0b0e11] p-4 rounded-2xl border border-gray-800/50 hover:border-blue-500/30 transition-all font-mono">
                ...
                <div class="flex justify-between text-[10px] mt-2">
                  ...
                  <span :class="item.currentPrice > item.buyPrice ? 'text-green-500' : 'text-red-500'" class="font-bold">
...
interface MiningItem { type: 'diamond' | 'gold' | 'bronze'; baseVal: number; }
...
const leaderboard = computed(() => {
  const user = { name: `${userProfile.value.name} (You)`, totalValue: userTotalValue.value, level: 'Investor', skill: 0, isUser: true };
  return [...bots.value, user].sort((a, b) => b.totalValue - a.totalValue);
});
const currentStockHolding = computed(() => portfolio.value.find(p => p.symbol === selectedStock.value.symbol)?.amount || 0);

// MINIGAME LOGIC: BLIND BOX (TÚI MÙ)
const throwHook = () => {
...
const pickRandomLoot = () => {
  ...
  // Hệ thống túi mù: 100% trúng quà khi thả câu
  ...
  // Random vật phẩm từ túi mù
  const lucky = lootPool[Math.floor(Math.random() * lootPool.length)];
  if (lucky) currentLoot.value = lucky;
  ...
  const retractInterval = setInterval(() => {
    ...
    if (hookLength.value <= 20) {
      ...
      if (currentLoot.value) currentView.value = 'Crushing';
    }
  }, 20);
};
...
const handleHit = (e: MouseEvent) => {
  ...
  setTimeout(() => moneyNotes.value.shift(), 800);
};

const handleStartInvestment = () => {
  if (userProfile.value.name.trim()) currentView.value = 'Market';
};
const goToView = (v: any) => currentView.value = v;
...
const buyStock = () => {
  ...
  if (balance.value >= cost) {
    ...
    if (existing) {
      existing.buyPrice = ((existing.amount * existing.buyPrice) + cost) / (existing.amount + tradeAmount.value);
      existing.amount += tradeAmount.value;
    } else {
      portfolio.value.push({ symbol: selectedStock.value.symbol, amount: tradeAmount.value, buyPrice: selectedStock.value.price, currentPrice: selectedStock.value.price });
    }
  }
};
...
const sellStock = () => {
  ...
  if (idx !== -1 && target) {
    ...
    if (target.amount <= 0) portfolio.value.splice(idx, 1);
  }
};
...
let marketInt: ReturnType<typeof setInterval>;
let hookSwingInt: ReturnType<typeof setInterval>;

onMounted(() => {
  window.addEventListener('keydown', (e) => {
    if (e.code === 'Space' && currentView.value === 'Mining') throwHook();
  });

  hookSwingInt = setInterval(() => {
    if (!isHookMoving.value) hookAngle.value = Math.sin(Date.now() / 600) * 50;
  }, 30);

  let lp = selectedStock.value.price;
  for (let i = 0; i < 40; i++) {
    const o = lp; const c = lp * (1 + (Math.random() * 0.04 - 0.02));
    candleData.value.push({ open: o, close: c, high: Math.max(o, c) * 1.01, low: Math.min(o, c) * 0.99 });
    lp = c;
  }
  ...
  marketInt = setInterval(() => {
    stocks.value.forEach(s => {
      ...
      if (p) p.currentPrice = s.price;

      if (s.symbol === selectedStock.value.symbol) {
        const last = candleData.value[candleData.value.length - 1];
        if (last) {
          const o = last.close; const c = s.price;
          candleData.value.shift();
          candleData.value.push({ open: o, close: c, high: Math.max(o, c) * 1.005, low: Math.min(o, c) * 0.995 });
        }
      }
    });
    bots.value.forEach(b => b.totalValue *= (1 + (b.skill + (Math.random() * 1 - 0.5)) / 100));
  }, 3000);
});

onUnmounted(() => {
  clearInterval(marketInt);
  clearInterval(hookSwingInt);
});
</script>
...
/* HIỆU ỨNG VẬT PHẨM TRÔI NỔI TRONG SƯƠNG MÙ */
@keyframes float {
