    ...
    <div v-else class="flex flex-col lg:flex-row gap-8 items-start">
      <div class="flex-1 min-w-0">
        ...
        >
          ...
          <p class="text-text-secondary font-display text-sm tracking-wide ml-7">
            Đang luyện tập
            <span class="text-accent-amber font-bold">{{ activeQuestions.length }}</span> câu hỏi
          </p>
        </div>
        ...
        <div class="space-y-8">
          ...
          >
            ...
            <div class="mb-6">
              <div class="flex items-center justify-between gap-2 mb-4">
                <div class="flex items-center gap-3">
                  <span
                    class="font-display text-accent-coral font-bold text-lg uppercase tracking-wider whitespace-nowrap"
                    >Câu {{ index + 1 }}:</span
                  >
                  <div
                    v-if="q.isCritical"
              ...
              <h3
                class="font-display text-xl sm:text-2xl font-semibold text-text-primary leading-snug"
              >
                {{ q.text }}
              </h3>
            </div>
            ...
            >
              <div
                class="font-display font-bold text-accent-amber mb-2 flex items-center gap-2 uppercase tracking-tight"
              >
      ...
      <div class="w-full lg:w-80 shrink-0 sticky top-8 z-10 flex flex-col gap-6">
        <div class="border border-border-default bg-bg-surface p-6 shadow-2xl relative">
          ...
          <div
            class="absolute top-0 right-0 w-24 h-24 bg-accent-amber/5 -rotate-12 translate-x-8 -translate-y-8 select-none pointer-events-none font-display font-black text-8xl opacity-10"
          >
            NAV
          </div>

          <h3
            class="font-display font-bold text-text-primary mb-6 flex items-center gap-2 text-sm uppercase tracking-widest border-b border-border-default pb-4"
          >
          ...
          <div class="mt-8 space-y-3 px-1 border-t border-border-default pt-6 border-dashed">
            <div
              class="flex items-center justify-between text-[10px] uppercase tracking-widest font-display font-bold"
            >
                ...
                >{{
                  Math.round(
                    (Object.keys(userAnswers).filter((id) =>
                      activeQuestions.some((q) => q.id === Number(id)),
                    ).length /
            ...
            <div class="h-1 bg-border-default w-full">
              <div
                ...
                :style="{
                  width: `${(Object.keys(userAnswers).filter((id) => activeQuestions.some((q) => q.id === Number(id))).length / activeQuestions.length) * 100}%`,
                }"
              ></div>
            </div>
            <div
              class="flex flex-col gap-2 text-[10px] text-text-dim font-display uppercase tracking-wider"
            >
              ...
              <div class="flex items-center gap-1.5 font-bold text-accent-coral/80">
                <span class="size-2 border border-accent-coral bg-bg-deep"></span> Câu điểm liệt
                (QUAN TRỌNG)
              </div>
            </div>
          </div>
