...
const scale = computed(() => {
  const maxWidth = props.params.span
  const maxHeight = props.params.sweep + props.params.rootChord
  const vScale = (VIEW_SIZE - 2 * PADDING) / Math.max(maxWidth, maxHeight)
  return vScale
})
...
const wingPoints = computed(() => {
  const { span, rootChord, tipChord, sweep, fuseWidth } = props.params
  const s = scale.value
...
const macIndicator = computed(() => {
  ...
  const macLeX = (params.sweep * (results.macDistance / ((params.span - params.fuseWidth) / 2))) * s
  
  const cx = center
  const cy = PADDING
  const halfFuse = (params.fuseWidth / 2) * s
...
const chassisCg = computed(() => {
  ...
  return {
    x: center,
    y: PADDING + props.params.chassisCog * s
  }
})
</script>
    ...
    <svg :viewBox="viewBox" class="w-full h-full drop-shadow-2xl">
      ...
      <defs>
        <pattern id="grid" width="20" height="20" patternUnits="userSpaceOnUse">
          <path d="M 20 0 L 0 0 0 20" fill="none" stroke="rgba(74, 97, 128, 0.1)" stroke-width="0.5"/>
        </pattern>
      </defs>
      ...
      <g opacity="0.4">
        <line
          :x1="macIndicator.lx" :y1="macIndicator.y"
          :x2="macIndicator.lx" :y2="macIndicator.y + macIndicator.l"
          stroke="var(--color-text-secondary)"
          stroke-width="1.5"
          stroke-dasharray="4 2"
        />
        <line
          :x1="macIndicator.x" :y1="macIndicator.y"
          :x2="macIndicator.x" :y2="macIndicator.y + macIndicator.l"
          stroke="var(--color-text-secondary)"
          stroke-width="1.5"
          stroke-dasharray="4 2"
        />
      ...
      <text :x="center" :y="PADDING - 10" text-anchor="middle" class="text-[10px] fill-text-dim uppercase tracking-widest font-display">Mũi</text>
      ...
      <g :transform="`translate(${targetCg.x}, ${targetCg.y})`">
        ...
        <text x="20" y="5" class="text-[10px] fill-accent-amber font-bold font-display uppercase tracking-widest">Mục tiêu</text>
      </g>

      <!-- Actual CG (Diamond) -->
      <g :transform="`translate(${actualCg.x}, ${actualCg.y})`">
        <rect x="-6" y="-6" width="12" height="12" fill="var(--color-accent-coral)" transform="rotate(45)" />
        <text x="-20" y="5" text-anchor="end" class="text-[10px] fill-accent-coral font-bold font-display uppercase tracking-widest">Thực tế</text>
      </g>

      <!-- Parts -->
      <g v-for="part in parts" :key="part.id">
        <rect
          :x="center - (part.width * scale / 2)"
          :y="PADDING + (part.x * scale) - (part.length * scale / 2)"
          :width="part.width * scale"
          :height="part.length * scale"
