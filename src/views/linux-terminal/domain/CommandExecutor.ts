...
export class CommandExecutor {
  ...
  private dispatch(input: string): OutputLine[] {
    ...
    switch (cmd) {
      ...
      case 'll':
        return this.cmdLs(['-la', ...args])
      case 'la':
        return this.cmdLs(['-a', ...args])
      case 'cd':
      ...
      case 'clear':
        return [line('info', '__CLEAR__')]
      case 'whoami':
        return [line('output', this.user)]
      case 'hostname':
        return [line('output', this.hostname)]
      case 'uname':
        return this.cmdUname(args)
      case 'date':
        return [line('output', new Date().toString())]
      case 'history':
      ...
      case 'cmatrix':
        return this.cmdCmatrix()
      case 'cowsay':
        return this.cmdCowsay(args)
      case 'fortune':
        return this.cmdFortune()
      case 'uptime':
      ...
      case 'top':
        return [
          line('info', 'top - interactive mode không được hỗ trợ trong terminal giả lập này.'),
        ]
      case 'vim':
      case 'nvim':
        return [
      ...
      case 'nano':
        return [line('info', 'nano: editor mode không được hỗ trợ ở đây.')]
      case 'sudo':
        return this.cmdSudo(args)
      case 'pacman':
        return this.cmdPacman(args)
      case 'apt':
      case 'apt-get':
        return [
          line(
            'error',
            `E: Lệnh 'apt' không tìm thấy. Bạn đang dùng Arch Linux btw. Hãy dùng 'pacman'.`,
          ),
        ]
      case 'brew':
        return [line('error', `brew: command not found. Đây là Linux, không phải macOS.`)]
      case 'ping':
      ...
      case 'node':
        return [
          line('info', 'Node.js v22.0.0\nWelcome to Node.js REPL (giả lập — chỉ in thông báo này)'),
        ]
      case 'python':
      case 'python3':
        return [line('info', 'Python 3.12.0\n>>> (giả lập — chỉ in thông báo này)')]
      case 'ssh':
        return [line('output', `ssh: connect to host ${args[0] ?? 'remote'}: Connection timed out`)]
      case 'man':
        return this.cmdMan(args[0])
      case 'which':
        return this.cmdWhich(args[0])
      case 'find':
      ...
      case 'chown':
        return [
          line('error', `chown: changing ownership of '${args[1] ?? ''}': Operation not permitted`),
        ]
      case 'alias':
        return this.cmdAlias(args)
      case 'exit':
        return [line('info', 'Thoát terminal... (F5 để refresh trang)')]
      case 'logout':
        return [line('info', 'logout')]
      case 'reboot':
        return [
          line(
            'info',
            'Broadcast message from user@vibe-machine:\nThe system is going down for reboot NOW!\n\n(Không thể reboot thật đâu, yên tâm 😄)',
          ),
        ]
      case 'shutdown':
        return [line('info', 'Shutdown scheduled... just kidding. 😄')]
      case 'htop':
        return [
          line(
            'info',
            'htop - interactive mode không được hỗ trợ.\nDùng "ps aux" để xem processes.',
          ),
        ]
      case 'tree':
        return this.cmdTree()
  ...
  private cmdHelp(): OutputLine[] {
    return [
      line(
        'output',
        `Hệ thống Linux Terminal giả lập - Các lệnh hỗ trợ:
  ...
  private cmdLs(args: string[]): OutputLine[] {
    ...
    if (longFmt) {
      ...
      const lines = nodes.map((n) => {
        const sizeStr =
          n.type === 'dir' ? '4096' : String(n.size ?? n.content?.length ?? 0).padStart(8)
  ...
  private cmdUname(args: string[]): OutputLine[] {
    if (args.includes('-a')) {
      return [
        line(
          'output',
          'Linux vibe-machine 6.7.0-arch1-1 #1 SMP PREEMPT_DYNAMIC Fri, 09 Mar 2026 07:00:00 +0000 x86_64 GNU/Linux',
        ),
      ]
    }
  ...
  private cmdUptime(): OutputLine[] {
    ...
    return [
      line(
        'output',
        ` ${new Date().toLocaleTimeString()} up ${h}:${String(m).padStart(2, '0')}, 1 user,  load average: 0.${Math.floor(Math.random() * 99)}, 0.${Math.floor(Math.random() * 99)}, 0.${Math.floor(Math.random() * 99)}`,
      ),
    ]
  }
  ...
  private cmdSudo(args: string[]): OutputLine[] {
    ...
    if (args[0] === 'su')
      return [line('output', '[sudo] password for user: \nWelcome, root! (giả lập)')]
    return [
      line(
        'error',
  ...
  private cmdGit(args: string[]): OutputLine[] {
    switch (args[0]) {
      ...
      case 'pull':
        return [line('output', 'Already up to date.')]
      case 'push':
        return [line('output', 'Everything up-to-date')]
      case 'add':
        return [line('output', '')]
      case 'commit':
        return [
          line(
            'output',
            `[main a1b2c3d] ${args.slice(2).join(' ')}\n 1 file changed, 1 insertion(+)`,
          ),
        ]
      case 'init':
        return [line('output', `Initialized empty Git repository in ${this.fs.pwd()}/.git/`)]
      default:
        return [line('output', 'usage: git [-v | --version] [-h | --help] <command> [<args>]')]
    }
  }
  ...
  private cmdGrep(args: string[]): OutputLine[] {
    ...
    if (file) {
      ...
      return matches.length ? [line('output', matches.join('\n'))] : []
    }
