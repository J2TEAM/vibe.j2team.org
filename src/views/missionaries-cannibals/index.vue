...
const leftHumans = computed(() => characters.value.filter(c => c.type === 'human' && c.side === 'left' && !c.onBoat))
const leftDemons = computed(() => characters.value.filter(c => c.type === 'demon' && c.side === 'left' && !c.onBoat))
const rightHumans = computed(() => characters.value.filter(c => c.type === 'human' && c.side === 'right' && !c.onBoat))
const rightDemons = computed(() => characters.value.filter(c => c.type === 'demon' && c.side === 'right' && !c.onBoat))
...
function canBoard(type: CharType) {
    ...
    const pool = boatSide.value === 'left' ?
...
function checkDanger() {
    if (!isSafe(leftHumans.value.length, leftDemons.value.length) ||
...
<div class="root" :class="{ mounted }">
    ...
    <div class="bg-hills">
        <svg viewBox="0 0 1440 180" preserveAspectRatio="none" class="hill-svg">
            <path d="M0,180 Q120,60 280,100 Q440,140 600,70 Q760,0 920,80 Q1080,160 1200,90 Q1320,20 1440,100 L1440,180Z" fill="#4caf50" />
            <path d="M0,180 Q100,90 240,120 Q380,150 520,90 Q660,30 820,100 Q980,170 1100,110 Q1220,50 1440,130 L1440,180Z" fill="#66bb6a" opacity="0.7" />
        </svg>
    </div>
    ...
    <div class="page">
        ...
        <header class="card reveal" style="--d:0">
            <div class="header-row">
                <div>
                    ...
                    <p class="sub">Đưa tất cả sang bờ phải — không để quỷ nhiều hơn người ở bất kỳ bờ nào!</p>
                </div>
                <div class="hbtns">
                    <RouterLink to="/" class="btn btn-sky home-btn" style="color: black;">
                        ← Trang chủ
                    </RouterLink>
                    <button class="btn btn-amber" style="color: black;" @click="initLevel(currentLevel)">↺ Chơi lại</button>
                    <button class="btn btn-sky" style="color: black;" @click="showRules = !showRules">📖 Luật chơi</button>
                </div>
            </div>
        </header>
        ...
        <Transition name="slide">
            <section v-if="showRules" class="card rules-panel">
                <div class="rules-grid">
                    ...
                    <div class="rc rc-gold"><span class="rc-ico">📊</span>
                        <div>
                            <h4>Các cấp độ</h4>
                            <ul>
        ...
        <div class="levels reveal" style="--d:1">
            <button v-for="(cfg,key) in levelConfigs" :key="key" class="lvl" :class="[`lvl-${key}`, { active: currentLevel===key }]" @click="initLevel(key as LevelKey)">
                <span class="lvl-name">{{ cfg.label }}</span>
                <span class="lvl-info">{{ cfg.missionaries }}🧑 {{ cfg.cannibals }}👹 · ⛵×{{ cfg.capacity }}</span>
            </button>
        </div>
        ...
        <div class="river-scene reveal" style="--d:2">
            ...
            <div class="river-illus">
                ...
                <!-- BOAT -->
                <div class="boat-container" :class="boatSide==='right' ? 'to-right' : 'to-left'" :style="isCrossing ? 'transition:left .95s cubic-bezier(.4,0,.2,1)' : 'transition:none'">
                    ...
                    <div class="boat" :class="{ rocking: isCrossing }">
                        ...
                        <svg class="hull" viewBox="0 0 130 58">
                            ...
                            <path d="M8,12 Q65,4 122,12 L110,50 Q65,57 20,50 Z" fill="url(#hg)" stroke="#6b3a0f" stroke-width="1.5" />
                            <path d="M14,14 Q65,8 116,14" fill="none" stroke="#f5c842" stroke-width="2" opacity=".6" />
                            <line x1="40" y1="14" x2="35" y2="49" stroke="#6b3a0f" stroke-width="0.7" opacity=".4" />
                            <line x1="65" y1="10" x2="65" y2="55" stroke="#6b3a0f" stroke-width="0.7" opacity=".4" />
                            <line x1="90" y1="14" x2="95" y2="49" stroke="#6b3a0f" stroke-width="0.7" opacity=".4" />
                            <path d="M20,50 Q65,57 110,50" fill="none" stroke="#5b3310" stroke-width="1.2" opacity=".5" />
                        </svg>
                        ...
                        <div class="boat-pax">
                            <TransitionGroup name="pax" tag="div" class="pax-row pax-humans">
                                <span v-for="ch in boatHumans" :key="ch.id" class="pax-char" :class="{ 'pax-boarding': ch.animBoarding, 'pax-alighting': ch.animAlighting }" @click="deboardChar('human')" title="↑ Xuống thuyền">🧑<span class="pax-hint">↑</span></span>
                            </TransitionGroup>
                            <TransitionGroup name="pax" tag="div" class="pax-row pax-demons">
                                <span v-for="ch in boatDemons" :key="ch.id" class="pax-char" :class="{ 'pax-boarding': ch.animBoarding, 'pax-alighting': ch.animAlighting }" @click="deboardChar('demon')" title="↑ Xuống thuyền">👹<span class="pax-hint">↑</span></span>
                            </TransitionGroup>
                        ...
                        <!-- Capacity dots -->
                        <div class="cap-row">
                            <span v-for="i in level.capacity" :key="i" class="cap-dot" :class="{ on: i<=boatCount }"></span>
                        </div>
                    </div>
            ...
            <div class="banks">
                    ...
                    <div class="chars-block">
                        ...
                        <div class="chars-row">
                            <TransitionGroup name="char" tag="div" class="chars-inner">
                                <div v-for="ch in leftHumans" :key="ch.id" class="char ch-human" :class="{ 'ch-boarding': ch.animBoarding }" @click="boardChar('human')" title="↓ Lên thuyền">
                        ...
                        <div class="chars-row">
                            <TransitionGroup name="char" tag="div" class="chars-inner">
                                <div v-for="ch in leftDemons" :key="ch.id" class="char ch-demon" :class="{ 'ch-boarding': ch.animBoarding }" @click="boardChar('demon')" title="↓ Lên thuyền">
                                    <span class="ch-em">👹</span>
                ...
                <div class="bank bank-right" :class="{ 'bank-win': hasWon }">
                    ...
                    <div class="chars-block">
                        <div class="chars-row">
                            <TransitionGroup name="char" tag="div" class="chars-inner">
                                <div v-for="ch in rightHumans" :key="ch.id" class="char ch-human" :class="{ 'ch-arriving': ch.animArriving, 'ch-boarding': ch.animBoarding }" @click="boatSide==='right' && boardChar('human')" :title="boatSide==='right' ? '↓ Lên thuyền' : ''">
                                    <span class="ch-em">🧑</span>
                                    <span class="ch-lbl">Người</span>
                        ...
                        <div class="chars-row">
                            <TransitionGroup name="char" tag="div" class="chars-inner">
                                <div v-for="ch in rightDemons" :key="ch.id" class="char ch-demon" :class="{ 'ch-arriving': ch.animArriving, 'ch-boarding': ch.animBoarding }" @click="boatSide==='right' && boardChar('demon')" :title="boatSide==='right' ? '↓ Lên thuyền' : ''">
                                    <span class="ch-em">👹</span>
                                    <span class="ch-lbl">Quỷ</span>
                                    <span v-if="boatSide==='right'" class="ch-hint">↓ Lên thuyền</span>
                                </div>
        ...
        <div class="card controls reveal" style="--d:3">
            <div class="ctrl-stats">
                <div class="stat"><span class="stat-l">Vị trí thuyền</span><span class="stat-v">{{ boatSide==='left' ? '◀ Bờ Trái' : 'Bờ Phải ▶' }}</span></div>
                <div class="stat"><span class="stat-l">Trên thuyền</span><span class="stat-v">{{ boatCount }}/{{ level.capacity }}</span></div>
                <div class="stat"><span class="stat-l">Số lượt</span><span class="stat-v steps-v">{{ steps }}</span></div>
            </div>

            <button class="cross-btn ripple-cont" :disabled="!canCross" @click="(e) => { addRipple(e as MouseEvent); crossRiver() }">
                <span v-if="!isCrossing">🚣 Qua Sông</span>
                <span v-else class="row-anim">⛵ Đang chèo<span class="dots"><b>.</b><b>.</b><b>.</b></span></span>
    ...
    <Transition name="overlay">
        <div v-if="showWin" class="win-overlay">
            <div class="win-box">
                ...
                <p class="win-sub">Level <strong>{{ level.label }}</strong> — <strong>{{ steps }} lượt</strong></p>
                <div class="win-parade">
                    <span v-for="i in level.missionaries" :key="'h'+i">🧑</span>
                    <span v-for="i in level.cannibals" :key="'d'+i">👹</span>
                </div>
                <div class="win-acts">
                    <button class="btn btn-green" style="color: black;" @click="initLevel(currentLevel)">↺ Chơi lại</button>
                    <button v-if="currentLevel!=='hard'" class="btn btn-sky" style="color: black;" @click="initLevel(currentLevel==='easy'?'medium':'hard')">Level tiếp →</button>
                </div>
            </div>
        </div>
    </Transition>
...
.sun {
    ...
    box-shadow: 0 0 40px rgba(253, 224, 71, .7), 0 0 80px rgba(253, 224, 71, .3);
    animation: glow 3s ease-in-out infinite alternate;
}

@keyframes glow {
    to {
        box-shadow: 0 0 60px rgba(253, 224, 71, .9), 0 0 110px rgba(253, 224, 71, .4)
    }
}
...
.oar.rowing {
    animation: rowOar .5s ease-in-out infinite alternate;
}
...
.rocking {
    animation: rock .95s ease-in-out;
}
...
.pax-boarding {
    animation: paxBoard .38s ease-in;
}

.pax-alighting {
    animation: paxAlight .35s ease-out;
}
...
.pax-enter-active {
    animation: paxIn .38s cubic-bezier(.34, 1.56, .64, 1);
}

.pax-leave-active {
    animation: paxOut .3s ease;
    position: absolute;
}
...
.cap-dot.on {
    background: #fde047;
    border-color: #facc15;
    box-shadow: 0 0 7px rgba(253, 224, 71, .8);
}
...
.bank-win {
    ...
    box-shadow: 0 0 24px rgba(74, 222, 128, .3) !important;
}
...
.ch-boarding {
    animation: jumpDown .4s cubic-bezier(.4, 0, .6, 1) forwards;
}
...
@keyframes jumpDown {
    ...
    100% {
        transform: translateY(55px) scale(.4);
        opacity: 0
    }
}
...
.ch-arriving {
    animation: dropDown .45s cubic-bezier(.34, 1.56, .64, 1);
}
...
.char-enter-active {
    animation: charIn .4s cubic-bezier(.34, 1.56, .64, 1);
}

.char-leave-active {
    animation: charOut .3s ease;
    position: absolute;
}
...
.win-parade {
    ...
    margin: .8rem 0;
    letter-spacing: .18em;
    animation: parade .6s ease infinite alternate;
}
