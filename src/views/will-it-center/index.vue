...
const levels: Level[] = [
  {
    ...
    targetHtml:
      '<div class="w-16 h-16 bg-accent-coral flex items-center justify-center font-display font-bold text-bg-deep shadow-lg">01</div>',
  ...
  },
  {
    ...
    targetHtml:
      '<div class="w-20 h-20 bg-accent-amber rounded-full flex items-center justify-center font-display font-bold text-bg-deep shadow-lg">02</div>',
  ...
  },
  {
    ...
    initialCode: '/* CSS của div ngoài */\n.container {\n  display: grid;\n  \n}',
    targetHtml:
      '<div class="w-24 h-24 bg-accent-sky rotate-12 flex items-center justify-center font-display font-bold text-bg-deep shadow-lg">03</div>',
  ...
  },
  {
    ...
    initialCode:
      '/* CSS của khối 04 (Lưu ý: container đã có position: relative) */\n.box {\n  position: absolute;\n  top: 50%;\n  left: 50%;\n  \n}',
    targetHtml:
      '<div class="box w-20 h-20 border-4 border-text-primary bg-bg-surface flex items-center justify-center font-display font-bold text-text-primary shadow-lg">04</div>',
  ...
  },
  {
    ...
    initialCode:
      '/* CSS của khối 05 (Lưu ý: container đã có position: relative) */\n.box {\n  position: absolute;\n  inset: 0;\n  \n}',
    targetHtml:
      '<div class="box w-16 h-16 bg-text-primary rounded-xl flex items-center justify-center font-display font-bold text-bg-deep shadow-2xl skew-x-12">05</div>',
  ...
  },
  {
    ...
    targetHtml:
      '<div class="w-full h-full flex"><div class="box w-20 h-20 bg-accent-sky rounded text-bg-deep flex items-center justify-center font-display font-bold text-xl shadow-[0_0_20px_theme(colors.accent.sky)]">06</div></div>',
  ...
  },
  {
    ...
    initialCode:
      '/* CSS của div ngoài. Note: Khối bên trong đã có display: inline-block */\n.container {\n  display: table-cell;\n  \n}',
    targetHtml:
      '<div class="w-24 h-24 bg-accent-coral border-2 border-bg-deep text-bg-deep font-display font-bold text-2xl rotate-45 inline-flex items-center justify-center shadow-lg"><span class="-rotate-45 block">07</span></div>',
  ...
  },
  {
    ...
    targetHtml:
      '<div class="w-full bg-border-default/20 border-y border-accent-amber font-display font-medium text-text-primary text-xl" style="height: 200px;">08</div>',
  ...
  },
  {
    ...
    initialCode:
      '/* CSS của khối 09 (Rộng 100px, Cao 100px). Container đã có position relative */\n.box {\n  position: absolute;\n  top: 50%;\n  left: 50%;\n  \n}',
    targetHtml:
      '<div class="box w-[100px] h-[100px] bg-accent-sky rounded-full flex items-center justify-center font-display font-bold text-bg-deep text-3xl shadow-[0_0_15px_theme(colors.accent.sky)]">09</div>',
  ...
  },
  {
    ...
    initialCode:
      '/* CSS của khối 10 (Rộng 128px, Cao 64px) Container đã có position relative */\n.box {\n  position: absolute;\n  \n}',
    targetHtml:
      '<div class="box w-[128px] h-[64px] bg-accent-coral flex items-center justify-center font-display font-bold text-bg-deep text-2xl shadow-xl skew-y-6">10</div>',
...
const checkSuccess = () => {
  ...
  const rulesMap: Record<number, () => boolean> = {
    ...
    3: () =>
      code.includes('place-items:center') ||
      (code.includes('justify-content:center') && code.includes('align-items:center')),
    4: () => code.includes('transform:translate(-50%,-50%)') || code.includes('translate:-50%-50%'),
    5: () =>
      code.includes('margin:auto') &&
      (code.includes('inset:0') || (code.includes('top:0') && code.includes('bottom:0'))),
    ...
    9: () =>
      (code.includes('margin-top:-50px') && code.includes('margin-left:-50px')) ||
      code.includes('margin:-50px00-50px') ||
      code.includes('margin:-50px'),
...
<template>
  <div
    class="min-h-screen bg-bg-deep text-text-primary font-body flex flex-col p-4 md:p-8 relative overflow-hidden"
  >
    <!-- Background Number -->
    <span
      class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-display text-[15rem] md:text-[25rem] font-bold text-accent-coral/5 select-none pointer-events-none -z-10 animate-fade-up"
    >
    ...
    <div class="max-w-6xl mx-auto w-full animate-fade-up">
      <!-- Header -->
      <header
        class="flex flex-col md:flex-row items-start justify-between mb-8 gap-4 border-b border-border-default pb-6"
      >
        <div>
        ...
        <div class="flex items-center gap-4">
          <span
            class="font-display text-text-dim text-sm tracking-widest bg-bg-surface px-4 py-2 border border-border-default"
          >
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full">
        ...
        <div class="flex flex-col gap-4">
          <div
            class="bg-bg-surface border border-border-default flex flex-col h-[400px] relative group"
          >
            <div
              class="bg-bg-elevated border-b border-border-default px-4 py-2 flex items-center justify-between"
            >
              <span class="font-display text-xs text-text-secondary tracking-widest"
                >STYLE.CSS</span
              >
            ...
            <div
              v-if="showHint"
              class="absolute bottom-4 right-4 max-w-xs bg-bg-elevated border-l-2 border-accent-sky p-3 text-sm text-text-secondary shadow-lg animate-fade-up z-10"
            >
        ...
        <div class="flex flex-col gap-4">
          <div
            ...
            :class="
              isSuccess
                ? 'border-accent-coral shadow-[0_0_30px_rgba(255,107,74,0.15)] pulse-border-once'
          ...
          >
            ...
            >
              ...
              <span
                v-if="isSuccess"
                class="font-display text-xs text-accent-coral tracking-widest animate-pulse"
                >CENTERED!</span
              >
            </div>
            ...
            <div class="w-full h-[calc(100%-37px)] p-4">
              ...
              >
                ...
                <div
                  class="absolute top-1/2 left-0 w-full border-t border-dashed border-border-default/30 -translate-y-px pointer-events-none"
                ></div>
                <div
                  class="absolute left-1/2 top-0 h-full border-l border-dashed border-border-default/30 -translate-x-px pointer-events-none"
                ></div>
                <div
                  class="absolute top-1/2 left-1/2 w-2 h-2 rounded-full bg-border-default/50 -translate-x-1/2 -translate-y-1/2 pointer-events-none"
                ></div>
              </div>
            </div>
            ...
            >
              <h2 class="font-display text-4xl font-bold text-accent-coral mb-6 text-center">
                HOÀN HẢO!
              </h2>
